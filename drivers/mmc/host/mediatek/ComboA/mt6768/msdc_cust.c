...
int msdc_of_parse(struct platform_device *pdev, struct mmc_host *mmc)
{
	...
	*/
	if (of_get_property(np, "fast_power_off", NULL)) {
		...
		//pr_notice("\n");
		if (of_get_property(np, "PD1901", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD2066F_EX", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD1987F_EX", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD1913F_EX_11.0", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD1934F_EX_11.0", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD2066B", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD2066", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
		} else if (of_get_property(np, "PD2103", NULL)) {
			//if (*(version + 2) == '0') {
				mmc->fast_power_off = true;
				pr_notice("[msdc%d]: in fast_power_off mode", mmc->index);
			//}
