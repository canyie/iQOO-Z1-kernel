	...
	struct mtk_ddp_config *cfg, unsigned int *gs)
{
	...
	/* adapt ultra threshold after fps above 120 */
	if (if_fps > 121) {
		pre_ultra_low_us = 180;
		pre_ultra_high_us = 200;
		ultra_low_us = 150;
		ultra_high_us = 180;
		urgent_low_us = 110;
		urgent_high_us = 120;
	...
	gs[GS_RDMA_TH_HIGH_FOR_SODI] = DIV_ROUND_UP(
		gs[GS_RDMA_FIFO_SIZE] * FP - (fill_rate > consume_rate ?
		(fill_rate - consume_rate) : (consume_rate - fill_rate)) * 12,
		FP);
