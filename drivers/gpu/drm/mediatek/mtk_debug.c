...
static atomic_t white_list_ref;
...
static int white_list_set(void *data, u64 val)
{
	atomic_set(&white_list_ref, val);
	return 0;
}

static int white_list_get(void *data, u64 *val)
{
	*val = atomic_read(&white_list_ref);
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(whiteList_fops, white_list_get, white_list_set, "%llu\n");

bool is_white_list_enable(void)
{
	return atomic_read(&white_list_ref);
}
...
void disp_dbg_probe(void)
{
	...
	d_folder = debugfs_create_dir("dispWhiteList", NULL);
	if (d_folder) {
		d_file = debugfs_create_file("whiteList", S_IFREG | 0644,
		d_folder, NULL, &whiteList_fops);
	}
