...
static struct stAF_DrvList g_stAF_DrvList[MAX_NUM_OF_LENS] = {
	{1, AFDRV_DW9781COISAF, DW9781COISAF_SetI2Cclient, DW9781COISAF_Ioctl,
	 DW9781COISAF_Release, DW9781COISAF_GetFileName, NULL},
	...
	if (Stage == 0) {
         ...
         mutex_lock(&dual_af_mutex);
          node = of_find_compatible_node(
           ...
           if (af_pinctrl_init(lens_device))
...
#if defined(CONFIG_MTK_CAM_PD2066A) || defined(CONFIG_MTK_CAM_PD2066BA) || defined(CONFIG_MTK_CAM_PD2066F_EX)
		if(strcmp(stMotorName.uMotorName,"GT9772V1AF") == 0)
			strcpy(stMotorName.uMotorName,"GT9772AF");
#endif
...
#if (!defined(CONFIG_MTK_LEGACY) && !defined(CONFIG_MTK_CAM_PD2083F_EX))
...
#ifdef CONFIG_MTK_CAM_PD2083F_EX
	ois_interface_create(g_pstAF_I2Cclient, lens_device, OISDRV_DW9781C);
#endif
	/*add end*/
	LOG_INF("The device is opened\n");
