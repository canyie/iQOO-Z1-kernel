...
#define OIS_DRVNAME "OIS_LC89129"
#define LOG_OIS_INF(format, args...)																					 \
	pr_info(OIS_DRVNAME " [%s][%d] " format, __func__, __LINE__, ##args)
#define LOG_OIS_ERR(format, args...)																					 \
	pr_err(OIS_DRVNAME " [%s][%d] " format, __func__, __LINE__, ##args)
...
static int RamRead32A(struct ois *ois, UINT_16 addr, UINT_32 *ReadVal)
{
	...
	*ReadVal = ((dataTemp & 0xFF) << 24) | (((dataTemp >> 8) & 0xFF) << 16) |
	...
	LOG_OIS_INF("zycdebug vRcvBuff = %x", *ReadVal);
	return re;
}
...
static int LC89129_set_target(struct ois *ois, void __user *user_buf)
{
	...
	default: { LOG_OIS_INF("default target"); }
	}
...
{
	int ret = 0;
	UINT_32 ReadVal1 = 0, ReadVal2 = 0;
	UINT_32 ReadVal3 = 0, ReadVal4 = 0;
	UINT_32 ReadVal5 = 0, ReadVal6 = 0;
	...
	ReadVal1 = ((((INT_32)ReadVal1) >> 16) & 0x0000FFFF);
	ReadVal2 = ((((INT_32)ReadVal2) >> 16) & 0x0000FFFF);
	// read hall raw data
	ret = RamRead32A(ois, 0x0178, &ReadVal3);
	ret = RamRead32A(ois, 0x017C, &ReadVal4);
	ReadVal3 = ((((INT_32)ReadVal3) >> 16) & 0x0000FFFF);
	ReadVal4 = ((((INT_32)ReadVal4) >> 16) & 0x0000FFFF);
	// read gyro gain data
	ret = RamRead32A(ois, 0x82B8, &ReadVal5);
	ret = RamRead32A(ois, 0x8318, &ReadVal6);
	LOG_OIS_INF("Gyro_raw_x = 0x%x, Gyro_raw_y = 0x%x,Hall_raw_x = 0x%x, Hall_raw_y = 0x%x, Gyro_gain(zoom)_x = 0x%x,Gyro_gain(zoom)_y = 0x%x",
		ReadVal1, ReadVal2, ReadVal3, ReadVal4, ReadVal5, ReadVal6);
	// read gyro raw data
	ret = RamRead32A(ois, 0x0448, &ReadVal1);
	ret = RamRead32A(ois, 0x0474, &ReadVal2);
	ReadVal1 = ((((INT_32)ReadVal1) >> 16) & 0x0000FFFF);
	ReadVal2 = ((((INT_32)ReadVal2) >> 16) & 0x0000FFFF);
	// read hall raw data
	ret = RamRead32A(ois, 0x0178, &ReadVal3);
	...
	ret = RamRead32A(ois, 0x82B8, &ReadVal5);
	ret = RamRead32A(ois, 0x8318, &ReadVal6);
	LOG_OIS_INF("ACC_raw_x = 0x%x, ACC_raw_y = 0x%x,Hall_raw_x = 0x%x, Hall_raw_y = 0x%x, Gyro_gain(zoom)_x = 0x%x,Gyro_gain(zoom)_y = 0x%x",
		ReadVal1, ReadVal2, ReadVal3, ReadVal4, ReadVal5, ReadVal6);
...
static void LC89129_vsync_process_fn(struct kthread_work *work)
{
	...
	LOG_OIS_INF("ois vsync processor E");
	...
	LOG_OIS_INF("ois vsync processor X %llu ms	vnum:%d",
