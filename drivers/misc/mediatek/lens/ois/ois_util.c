...
int ois_i2c_write_32(struct i2c_client *client, u16 addr, u32 data)
{
	...
	char           buf[6]  = {0,};
	int            ret     = 0;
	LOG_OIS_ERR("zycdebug : 0x%04x	 0x%08x", addr, data);
	...
	LOG_OIS_ERR("zycdebug : %d	 %d   %d	%d", buf[2], buf[3], buf[4], buf[5]);
		...
		const u8 *data, size_t size)
{
	...
	int            ret           = 0;
	u8             buf[514]      = {0,};
	u16            max_read_size = ARRAY_SIZE(buf) - 2;

	buf[0] = (addr & 0xFF00) >> 8;
	buf[1] = (addr & 0x00FF);

	if (size > max_read_size) {
		LOG_OIS_ERR("write out of buffer(exp=%d max=%d)", size, max_read_size);
		return -1;
	}
