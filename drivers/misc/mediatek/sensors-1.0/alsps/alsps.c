...
int als_extension_data_report_t(als_event_t alsdata, int status, int64_t time_stamp)
{
	...
	event.word[5] = alsdata.value[5];
	event.word[6] = (int32_t)alsdata.id;
	event.word[7] = (int32_t)alsdata.angle_x;
	event.word[8] = (int32_t)alsdata.angle_y;
	event.word[9] = (int32_t)alsdata.angle_z;
	event.word[10] = (int32_t)alsdata.motion;
	event.status = status;
	pr_err("%s als_data: %d %d %d %d %d %d id:%d angle:%d %d %d motion:%d\n", __func__,
		event.word[0], event.word[1], event.word[2], event.word[3], event.word[4],
		event.word[5], event.word[6], event.word[7], event.word[8], event.word[9], event.word[10]);
	err = sensor_input_event(cxt->als_mdev.minor, &event);
