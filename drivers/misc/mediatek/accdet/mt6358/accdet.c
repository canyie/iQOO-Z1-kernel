...
int accdet_r_irq;
int r_gpiopin = -1;
unsigned int g_rgpio_headset_deb;
...
static void dis_micbias_work_callback(struct work_struct *work)
{
	if (cable_type == HEADSET_NO_MIC) {
		...
		disable_accdet();
	pr_info("%s more than 3s,MICBIAS:Disabled c_type:0x%x\n",
		__func__, cable_type);
	}
}
#endif /* end of #if PMIC_ACCDET_KERNEL */
			 ...
			 if (cable_type == NO_DEVICE) {
				 ...
				 for (i = 0; i < 35; i++) {
				   ...
				   if (gpio_get_value(gpiopin)) {
					gpio_set_debounce(gpiopin, accdet_dts.plugout_deb);
				 ...
				 if (cable_type == NO_DEVICE) {
					...
					if (gpio_is_valid(r_gpiopin)) {
						...
						if ((hphl_val == 0) && (hphr_val == 0)) {
							printk("[headset-legen] %s ACCDET no work\n", __FUNCTION__);
							accdet_no_work = 1;
			 ...
			 if (cable_type == NO_DEVICE) {
				 ...
				 for (i = 0; i < 35; i++) {
				   ...
				   if (gpio_get_value(r_gpiopin)) {
					gpio_set_debounce(r_gpiopin, accdet_dts.plugout_deb);
...
static inline int ext_eint_setup(struct platform_device *platform_device)
{
	...
	if (!gpio_is_valid(r_gpiopin)) {
		pr_notice("right gpio detect not found\n");
		return 0;
	}
