...
#include "../../../../../vivo/system/display/vivosrc/fb/pd2103_djn658_nt36672a_fhdplus_video_cmd.h"
...
static void lcm_suspend_power(void)
{
	...
	MDELAY(5);
	lcm_bkg_setting(0);
	LCM_INFO("[LCM]lcm_suspend_power nt26672a- vddi keep high level\n");
	panel_reset_state = 0;
	LCM_INFO("\n");
}
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	if (level == 0) {
		push_table(handle, lcm_cmd_backlight_dimming_disable, sizeof(lcm_cmd_backlight_dimming_disable) / sizeof(struct LCD_setting_table), 1);
		dimming_enable = 0;
		LCM_INFO("LastBacklight = %d, dimming_enable=%d, high_bit=0x%x, low_bit=0x%x\n", bl_brightness_hal, dimming_enable, bl_level_12bit[0].para_list[0], bl_level_12bit[0].para_list[1]);
		MDELAY(40);
	}
...
{
	if (lcm_software_id == 0x49)
		return 0x30;
	else
		return 0xFF;
}

struct LCM_DRIVER pd2103_djn658_nt36672a_fhdplus_lcm_drv = {
	.name = "pd2103_djn658_nt36672a_fhdplus_drv",
