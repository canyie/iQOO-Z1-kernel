...
#define LOG_1 LOG_INF("OV02B10,MIPI 1LANE\n")
/***************************   Modify end    *******************************/
...
extern int ov02b10_otp_read(void);
extern otp_error_code_t OV02B10_OTP_ERROR_CODE;
MUINT32  sn_inf_main_ov02b10[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_ov02b10[4];  
...
static kal_uint32 test_i2c_is_timeout1()
{
	...
	LOG_INF("[test_i2c_is_timeout1]:[%s ] E \n",__FUNCTION__);
...
//pr_info(PFX "[%s ] E \n",__FUNCTION__);
	...
	//pr_err(PFX "[%s ] ret =%d \n",__FUNCTION__,ret);
	LOG_INF("[test_i2c_is_timeout1]:[%s ] ret =%d \n",__FUNCTION__,ret);
    return (ret==-2)?1:0;
}
...
//vivo lishuo add MTK patch for [H202012312161] 20210218 begin
    if (test_i2c_is_timeout1()) {    
	 ...
	 LOG_INF("[get_imgsensor_id]:[%s ] iic  timeout \n",__FUNCTION__);
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = ov02b10_otp_read();
				printk("zhanghainan read eeprom ---vivo_otp_read_when_power_on = %d,OV02B10_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, OV02B10_OTP_ERROR_CODE);
				/*vivo lxd add end*/
				return ERROR_NONE;
			}
			LOG_INF
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
	    printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  OV02B10_OTP_ERROR_CODE:%d \n", *feature_data,OV02B10_OTP_ERROR_CODE);
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = OV02B10_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_ov02b10, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_ov02b10, sizeof(MUINT32)*4); 
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_ov02b10[%d]= 0x%x\n", i, sn_inf_main_ov02b10[i]);
						}
				#endif
			}
				break;
