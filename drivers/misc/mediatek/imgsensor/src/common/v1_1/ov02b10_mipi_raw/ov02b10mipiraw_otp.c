...
#include "ov02b10mipi_Sensor.h"

#define PFX "OV02B10_EEPROM_OTP"
...
static unsigned char vivo_otp_data_ov02b10[VIVO_OTP_DATA_SIZE];
...
#endif
static int checksum;
otp_error_code_t OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_main_ov02b10[13];
extern MUINT32  crosstalk_main_ov02b10[299];
extern MUINT32  material_inf_main_ov02b10[4];  
...
int ov02b10_otp_read(void)
{
	...
	OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	/* This operation takes a long time, we need to skip it. guojunzheng add begin */
	if (is_atboot == 1) {
		printk("[lxd++]AT mode skip ov02b10_otp_read\n");
		return 1;
	}
	...
	for (i = 0; i < VIVO_OTP_DATA_SIZE; i++) {
		if (!vivo_read_eeprom(offset,  &vivo_otp_data_ov02b10[i])) {
			printk("[lxd++]read_vivo_eeprom 0x%0x %d fail \n", offset,  vivo_otp_data_ov02b10[i]);
			return 0;
		}
	...
	#if 0
	for(i = 0; i < VIVO_OTP_DATA_SIZE; i++) {
		printk("[lxd++]read_vivo_eeprom Data[0x%0x]:0x%x\n", i,  vivo_otp_data_ov02b10[i]);
	}
	#endif
	/*check_if_group_valid = vivo_otp_data_ov02b10[ModuleInfo_addr] | vivo_otp_data_ov02b10[Awb_addr] | vivo_otp_data_ov02b10[Af_addr] | vivo_otp_data_ov02b10[Lsc_addr] | vivo_otp_data_ov02b10[PD_Proc1_addr] | vivo_otp_data_ov02b10[PD_Proc2_addr];*/
	if ((0x01 == vivo_otp_data_ov02b10[ModuleInfo_addr]) && 
		(0x01 == vivo_otp_data_ov02b10[Fulse_id_addr]) && 
		(0x01 == vivo_otp_data_ov02b10[SN_addr]) && 
		(0x01 == vivo_otp_data_ov02b10[Awb_addr]) &&
		(0x01 == vivo_otp_data_ov02b10[Lsc_addr]))
		 {
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		/****ModuleInfo****/
		if (( 0x04!= vivo_otp_data_ov02b10[0x01])&&( 0x03!= vivo_otp_data_ov02b10[0x01]) ) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("[lxd++]Module ID error!!!    otp_error_code:%d\n", OV02B10_OTP_ERROR_CODE);
			return 0;
		} else if (((0x10 != vivo_otp_data_ov02b10[0x08])&&(0x14 != vivo_otp_data_ov02b10[0x08]))|| 
				(0x00 != vivo_otp_data_ov02b10[0x09]) || 
				(0x00 != vivo_otp_data_ov02b10[0x0A]) || 
				(0x03 != vivo_otp_data_ov02b10[0x02])) {
				
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("[lxd++]: Platform ID or Lens or VCM ID or Driver_IC ID  Error!!!    otp_error_code:%d\n", OV02B10_OTP_ERROR_CODE);
			return 0;
		} else if ((0xff != vivo_otp_data_ov02b10[0x0B]) || 
				(0x00 != vivo_otp_data_ov02b10[0x0C]) || 
				(0x0b != vivo_otp_data_ov02b10[0x0D]) || 
				((0x01 != vivo_otp_data_ov02b10[0x0E]) && (0x03 != vivo_otp_data_ov02b10[0x0E]) && (0x02 != vivo_otp_data_ov02b10[0x0E]))) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("[lxd++]: calibration version  Error!!!    Read version:0x%2x%2x%2x%2x\n", 
				vivo_otp_data_ov02b10[0x0B], vivo_otp_data_ov02b10[0x0C], vivo_otp_data_ov02b10[0x0D], vivo_otp_data_ov02b10[0x0E]);
			return 0;
		}
		/****ModuleInfo_checksum****/
		checksum = 0;
		for (i = ModuleInfo_addr+1; i < ModuleInfo_checksum_addr; i++) {
			checksum += vivo_otp_data_ov02b10[i];
			}
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_ov02b10[ModuleInfo_checksum_addr] != checksum) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("[lxd++]ModuleInfo_checksum error!!!   otp_error_code:%d\n", OV02B10_OTP_ERROR_CODE);
			return 0;
			}
		
		/****material info start****/
			material_inf_main_ov02b10[0] = (MUINT32)vivo_otp_data_ov02b10[0x0F];
			material_inf_main_ov02b10[1] = (MUINT32)vivo_otp_data_ov02b10[0x10];
			material_inf_main_ov02b10[2] = (MUINT32)vivo_otp_data_ov02b10[0x11];
			material_inf_main_ov02b10[3] = (MUINT32)vivo_otp_data_ov02b10[0x12];
			LOG_INF("material_inf_main_ov02b10[0] = 0x%x, material_inf_main_ov02b10[1] = 0x%x, material_inf_main_ov02b10[2] = 0x%x,material_inf_main_ov02b10[3] = 0x%x\n",
				material_inf_main_ov02b10[0]  , material_inf_main_ov02b10[1],  material_inf_main_ov02b10[2], material_inf_main_ov02b10[3]);
		/****material info end****/
		...
		for (i = Fulse_id_addr+1; i < Fulse_id_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_ov02b10[Fulse_id_checksum_addr] != checksum) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = SN_addr+1; i < SN_checksum_addr; i++) {
			/*LOG_INF("vivo_otp_data_ov02b10[0x%x] = 0x%x\n", i, vivo_otp_data_ov02b10[i]);*/
			checksum += vivo_otp_data_ov02b10[i];
			}
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_ov02b10[SN_checksum_addr] != checksum) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("SN_checksum error!!!   otp_error_code:%d\n", OV02B10_OTP_ERROR_CODE);
			return 0;
		}
		sn_inf_main_ov02b10[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_main_ov02b10[i+1] = (MUINT32)vivo_otp_data_ov02b10[i + SN_addr+1];
			/*LOG_INF("sn_inf_main_ov02b10[%d] = 0x%x, vivo_otp_data_ov02b10[0x%x] = 0x%x\n", i+1  , sn_inf_main_ov02b10[i+1],  i +SN_addr+1, vivo_otp_data_ov02b10[i+SN_addr+1]);*/
		}  
#if 0
 /****crosstalk_checksum****/
		checksum = 0;
		for (i = crosstalk_addr+1; i < crosstalk_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_ov02b10[crosstalk_checksum_addr] != checksum) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("crosstalk_checksum error!!!   otp_error_code:%d\n", OV02B10_OTP_ERROR_CODE);
			return 0;
		}

		crosstalk_main_ov02b10[0] = 0x01;
		for (i = 0; i < 288; i++) {
			crosstalk_main_ov02b10[i+1] = (MUINT32)vivo_otp_data_ov02b10[i + crosstalk_addr+1];
			/*LOG_INF("crosstalk_main_ov02b10[%d] = 0x%x, vivo_otp_data_ov02b10[0x%x] = 0x%x\n", i+1  , crosstalk_main_ov02b10[i+1],  i +crosstalk_addr+1, vivo_otp_data_ov02b10[i+crosstalk_addr+1]);*/
		}
		...
		for (i = Af_addr+1; i < Af_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_ov02b10[Af_checksum_addr] != checksum) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Awb_addr+1; i < Awb_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_ov02b10[Awb_checksum_addr] != checksum) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Lsc_addr+1; i < Lsc_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_ov02b10[Lsc_checksum_addr] != checksum) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = PDAF_proc1_addr+1; i < PDAF_proc1_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (vivo_otp_data_ov02b10[PDAF_proc1_checksum_addr] != checksum) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("[lxd++]PDAF_data_checksum error!!!   otp_error_code:%d\n", OV02B10_OTP_ERROR_CODE);
			return 0;
			}
		...
		/****PDAF Proc2_checksum****/
		if (0x01 == vivo_otp_data_ov02b10[PDAF_proc2_addr]){
			...
			for (i = PDAF_proc2_addr+1; i < PDAF_proc2_checksum_addr; i++) {
				...
				checksum = checksum % 0xff+1;
			if (vivo_otp_data_ov02b10[PDAF_proc2_checksum_addr] != checksum) {
				OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
				LOG_INF("[lxd++]PDAF_data_checksum error!!!   otp_error_code:%d\n", OV02B10_OTP_ERROR_CODE);
				return 0;
				}
		...
		R_unit = vivo_otp_data_ov02b10[Awb_addr+1];
		R_unit = (R_unit << 8) | (vivo_otp_data_ov02b10[Awb_addr+2]);
		B_unit = vivo_otp_data_ov02b10[Awb_addr+3];
		B_unit = (B_unit << 8) | (vivo_otp_data_ov02b10[Awb_addr+4]);
		G_unit = vivo_otp_data_ov02b10[Awb_addr+5];
		G_unit = (G_unit << 8) | (vivo_otp_data_ov02b10[Awb_addr+6]);

		R_golden = vivo_otp_data_ov02b10[Awb_addr+7];
		R_golden = (R_golden << 8) | (vivo_otp_data_ov02b10[Awb_addr+8]);
		B_golden = vivo_otp_data_ov02b10[Awb_addr+9];
		B_golden = (B_golden << 8) | (vivo_otp_data_ov02b10[Awb_addr+10]);
		G_golden = vivo_otp_data_ov02b10[Awb_addr+11];
		G_golden = (G_golden << 8) | (vivo_otp_data_ov02b10[Awb_addr+12]);
		...
		if (t > 0) {
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("[lxd++]AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, OV02B10_OTP_ERROR_CODE);
			return 0;
		}
#if 0
		/****check if awb out of range[3000K low cct]****/
		R_unit_low = vivo_otp_data_ov02b10[Awb_addr+13];
		R_unit_low = (R_unit_low << 8) | (vivo_otp_data_ov02b10[Awb_addr+14]);
		B_unit_low = vivo_otp_data_ov02b10[Awb_addr+15];
		B_unit_low = (B_unit_low << 8) | (vivo_otp_data_ov02b10[Awb_addr+16]);
		G_unit_low = vivo_otp_data_ov02b10[Awb_addr+17];
		G_unit_low = (G_unit_low << 8) | (vivo_otp_data_ov02b10[Awb_addr+18]);

		R_golden_low = vivo_otp_data_ov02b10[Awb_addr+19];
		R_golden_low = (R_golden_low << 8) | (vivo_otp_data_ov02b10[Awb_addr+20]);
		B_golden_low = vivo_otp_data_ov02b10[Awb_addr+21];
		B_golden_low = (B_golden_low << 8) | (vivo_otp_data_ov02b10[Awb_addr+22]);
		G_golden_low = vivo_otp_data_ov02b10[Awb_addr+23];
		G_golden_low = (G_golden_low << 8) | (vivo_otp_data_ov02b10[Awb_addr+24]);
		...
		AF_inf  =  vivo_otp_data_ov02b10[Af_addr + 9];
		AF_inf = (AF_inf << 8) | (vivo_otp_data_ov02b10[Af_addr + 10]);

		AF_mac = vivo_otp_data_ov02b10[Af_addr + 11];
		AF_mac = (AF_mac << 8) | (vivo_otp_data_ov02b10[Af_addr + 12]);
		...
		if (diff_inf_macro > 480 || diff_inf_macro < 200) {  /*AF code out of range*/
			OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			LOG_INF("[lxd++]AF out of range error!!!   otp_error_code:%d    Module ID = 0x%0x   inf = %d    mac = %d\n", OV02B10_OTP_ERROR_CODE, vivo_otp_data_ov02b10[0x01], AF_inf, AF_mac);
			return 0;
		}
		#endif
		...
		{
			if(i == 0)
				LOG_INF("[lxd++]read_OV02B10_PDAF_data");
			if(i < 496)
				PDAF_data[i] = vivo_otp_data_ov02b10[PD_Proc1_addr+i+1];
			else //i >= 496
				PDAF_data[i] = vivo_otp_data_ov02b10[PD_Proc1_addr+i+3];
		}*/
		/*copy pdaf data*/
		/*memcpy(PDAF_data, &vivo_otp_data_ov02b10[PDAF_addr+1], PDAF_checksum_addr-PDAF_addr-1);*/
		/*lxd add for pdaf data end*/
		OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
		return 1;
	} else {
		OV02B10_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
