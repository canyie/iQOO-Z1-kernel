...
extern MUINT32  crosstalk_main_ov32alq[299];
...
int ov32alq_otp_read(void)
{
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		crosstalk_main_ov32alq[0] = 0x01;
		for (i = 0; i < 288; i++) {
			crosstalk_main_ov32alq[i+1] = (MUINT32)vivo_otp_data_ov32alq[i + crosstalk_addr+1];
			/*LOG_INF("crosstalk_main_ov32alq[%d] = 0x%x, vivo_otp_data_ov32alq[0x%x] = 0x%x\n", i+1  , crosstalk_main_ov32alq[i+1],  i +crosstalk_addr+1, vivo_otp_data_ov32alq[i+crosstalk_addr+1]);*/
		}
#if 0
		/****AF_checksum****/
		checksum = 0;
