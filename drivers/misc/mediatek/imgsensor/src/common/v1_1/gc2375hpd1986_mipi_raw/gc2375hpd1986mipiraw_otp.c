...
#define PFX "SUB2[2375]_EEPROM_OTP"
...
#define VIVO_SUB2_OTP_DATA_SIZE 0x19DF /*sizeof(moduleinfo)+sizeof(awb)+sizeof(af)+sizeof(lsc)*/
/*#define AF_RANGE_GOT*/  /****if got the range of AF_inf and AF_mac , open this define!!!!****/
#define VIVO_SUB2_EEPROM_WRITE_ID 0xA4
#define VIVO_SUB2_I2C_SPEED 400
#define VIVO_SUB2_MAX_OFFSET 0x19DE
...
#endif
static int checksum;
otp_error_code_t GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_sub2_gc2375hpd1986[13];
...
int SUB2_2375_otp_read(void)
{
	...
	GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	#if 1
	...
	for (i = 0; i < VIVO_SUB2_OTP_DATA_SIZE; i++) {
		if (!vivo_read_eeprom(offset,  &sub2_otp_data_gc2375hpd1986[i])) {
			LOG_INF("read_vivo_eeprom 0x%0x %d fail \n", offset,  sub2_otp_data_gc2375hpd1986[i]);
			return 0;
		}
	...
	/*for(i = 0; i < VIVO_OTP_DATA_SIZE; i++) {
		LOG_INF("read_vivo_eeprom Data[0x%0x]:0x%x\n", i,  sub2_otp_data_gc2375hpd1986[i]);
	}*/

	/*check_if_group_valid = sub2_otp_data_gc2375hpd1986[ModuleInfo_addr] | sub2_otp_data_gc2375hpd1986[Awb_addr] | sub2_otp_data_gc2375hpd1986[Af_addr] | sub2_otp_data_gc2375hpd1986[Lsc_addr] | sub2_otp_data_gc2375hpd1986[PD_Proc1_addr] | sub2_otp_data_gc2375hpd1986[PD_Proc2_addr];*/
	if ((0x01 == sub2_otp_data_gc2375hpd1986[ModuleInfo_addr]) &&
		(0x01 == sub2_otp_data_gc2375hpd1986[SN_addr]) && 
		(0x01 == sub2_otp_data_gc2375hpd1986[Awb_addr]) && 
		(0x01 == sub2_otp_data_gc2375hpd1986[Lsc_addr])  ) {
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		/****ModuleInfo****/
		if (  (VIVO_VENDOR_SUNWIN != sub2_otp_data_gc2375hpd1986[0x01])) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("Module ID error!!!    otp_error_code:%d\n", GC2375HPD1986_OTP_ERROR_CODE);
			return 0;
		} else if (((VIVO_SUB2_VENDOR_LENS_ID != sub2_otp_data_gc2375hpd1986[0x0008])) || 
			(VIVO_SUB2_VENDOR_VCM_ID != sub2_otp_data_gc2375hpd1986[0x0009]) || 
			(VIVO_SUB2_VENDOR_DRIVERIC_ID != sub2_otp_data_gc2375hpd1986[0x000A]) || 
			(VIVO_SUB2_VENDOR_PLATFORM_ID != sub2_otp_data_gc2375hpd1986[0x0002])) {
			
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF(": Platform ID or Lens or VCM ID or Driver_IC ID  Error!!!    otp_error_code:%d\n", GC2375HPD1986_OTP_ERROR_CODE);
			return 0;
		} else if ((0xff != sub2_otp_data_gc2375hpd1986[0x000B]) || 
			(0x00 != sub2_otp_data_gc2375hpd1986[0x000C]) || 
			(0x0b != sub2_otp_data_gc2375hpd1986[0x000D]) || 
			(0x01 != sub2_otp_data_gc2375hpd1986[0x000E])) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF(": calibration version  Error!!!    Read version:0x%2x%2x%2x%2x\n", 
				sub2_otp_data_gc2375hpd1986[0x000B], sub2_otp_data_gc2375hpd1986[0x000C], sub2_otp_data_gc2375hpd1986[0x000D], sub2_otp_data_gc2375hpd1986[0x000E]);
			return 0;
		}
		...
		for (i = ModuleInfo_addr+1; i < ModuleInfo_checksum_addr; i++) {
			checksum += sub2_otp_data_gc2375hpd1986[i];
			}
			checksum = checksum % 0xff+1;
		if (sub2_otp_data_gc2375hpd1986[ModuleInfo_checksum_addr] != checksum) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Fuse_id_addr+1; i < Fuse_id_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (sub2_otp_data_gc2375hpd1986[Fuse_id_checksum_addr] != checksum) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = SN_addr+1; i < SN_checksum_addr; i++) {
			/*LOG_INF("sub2_otp_data_gc2375hpd1986[0x%x] = 0x%x\n", i, sub2_otp_data_gc2375hpd1986[i]);*/
			checksum += sub2_otp_data_gc2375hpd1986[i];
			}
			checksum = checksum % 0xff+1;
		if (sub2_otp_data_gc2375hpd1986[SN_checksum_addr] != checksum) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("SN_checksum error!!!   otp_error_code:%d\n", GC2375HPD1986_OTP_ERROR_CODE);
			return 0;
		}
		sn_inf_sub2_gc2375hpd1986[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_sub2_gc2375hpd1986[i +1] = (MUINT32)sub2_otp_data_gc2375hpd1986[i + SN_addr+1];
			/*LOG_INF("sn_inf_sub2_gc2375hpd1986[%d] = 0x%x, sub2_otp_data_gc2375hpd1986[0x%x] = 0x%x\n", i+1  , sn_inf_sub2_gc2375hpd1986[i +1],  i +SN_addr+1, sub2_otp_data_gc2375hpd1986[i + SN_addr+1]);*/
		}
		...
		for (i = Awb_addr+1; i < Awb_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (sub2_otp_data_gc2375hpd1986[Awb_checksum_addr] != checksum) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Af_addr+1; i < Af_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (sub2_otp_data_gc2375hpd1986[Af_checksum_addr] != checksum) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Lsc_addr+1; i < Lsc_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (sub2_otp_data_gc2375hpd1986[Lsc_checksum_addr] != checksum) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = PDAF_proc1_addr+1; i < PDAF_proc1_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (sub2_otp_data_gc2375hpd1986[PDAF_proc1_checksum_addr] != checksum) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = PDAF_proc2_addr+1; i < PDAF_proc2_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (sub2_otp_data_gc2375hpd1986[PDAF_proc2_checksum_addr] != checksum) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("PDAF_data_checksum error!!!   otp_error_code:%d\n", GC2375HPD1986_OTP_ERROR_CODE);
			return 0;
		}
		...
		R_unit = sub2_otp_data_gc2375hpd1986[Awb_addr+1];
		R_unit = (R_unit << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+2]);
		B_unit = sub2_otp_data_gc2375hpd1986[Awb_addr+3];
		B_unit = (B_unit << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+4]);
		G_unit = sub2_otp_data_gc2375hpd1986[Awb_addr+5];
		G_unit = (G_unit << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+6]);

		R_golden = sub2_otp_data_gc2375hpd1986[Awb_addr+7];
		R_golden = (R_golden << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+8]);
		B_golden = sub2_otp_data_gc2375hpd1986[Awb_addr+9];
		B_golden = (B_golden << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+10]);
		G_golden = sub2_otp_data_gc2375hpd1986[Awb_addr+11];
		G_golden = (G_golden << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+12]);
		...
		if (t > 0) {
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, GC2375HPD1986_OTP_ERROR_CODE);
			return 0;
		}
		/****check if awb out of range[low cct]****/
		R_unit_low = sub2_otp_data_gc2375hpd1986[Awb_addr+13];
		R_unit_low = (R_unit_low << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+14]);
		B_unit_low = sub2_otp_data_gc2375hpd1986[Awb_addr+15];
		B_unit_low = (B_unit_low << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+16]);
		G_unit_low = sub2_otp_data_gc2375hpd1986[Awb_addr+17];
		G_unit_low = (G_unit_low << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+18]);

		R_golden_low = sub2_otp_data_gc2375hpd1986[Awb_addr+19];
		R_golden_low = (R_golden_low << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+20]);
		B_golden_low = sub2_otp_data_gc2375hpd1986[Awb_addr+21];
		B_golden_low = (B_golden_low << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+22]);
		G_golden_low = sub2_otp_data_gc2375hpd1986[Awb_addr+23];
		G_golden_low = (G_golden_low << 8) | (sub2_otp_data_gc2375hpd1986[Awb_addr+24]);
		...
		AF_inf  =  sub2_otp_data_gc2375hpd1986[Af_addr+2];
		AF_inf = (AF_inf << 8) | (sub2_otp_data_gc2375hpd1986[Af_addr+3]);

		AF_mac = sub2_otp_data_gc2375hpd1986[Af_addr+4];
		AF_mac = (AF_mac << 8) | (sub2_otp_data_gc2375hpd1986[Af_addr+5]);
		...
		if (diff_inf > 70 || diff_mac > 80 || diff_inf_macro > 450 || diff_inf_macro < 250) {  /*AF code out of range*/
			GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
		...
		/*for(i = 0;i < 1372;i++)
		{
			...
			if(i < 496)
				PDAF_data[i] = sub2_otp_data_gc2375hpd1986[PD_Proc1_addr+i+1];
			else //i >= 496
				PDAF_data[i] = sub2_otp_data_gc2375hpd1986[PD_Proc1_addr+i+3];
		}*/
		/*copy pdaf data*/
		/*memcpy(PDAF_data, &sub2_otp_data_gc2375hpd1986[PDAF_addr+1], PDAF_checksum_addr-PDAF_addr-1);*/
		/*cfx add for pdaf data end*/
		GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
		return 1;
	} else {
		GC2375HPD1986_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
