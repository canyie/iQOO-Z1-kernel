...
#include "ov13b10v1mipiraw_Sensor.h"

#define PFX "OV13B10V1_EEPROM_OTP"
...
otp_error_code_t OV13B10V1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int ov13b10v1_flag;
extern MUINT32  sn_inf_main_ov13b10v1[13];  /*0 flag   1-12 data*/
extern MUINT32  material_inf_main_ov13b10v1[4];  
bool ov13b10v1_vivo_read_eeprom(kal_uint16 addr,  BYTE *data)
{
...
int ov13b10v1_vivo_otp_read(void)
{
	int i = 0;
	...
	OV13B10V1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	for(i = 0; i < VIVO_OTP_DATA_SIZE; i++) {
		if(!ov13b10v1_vivo_read_eeprom(offset, &vivo_otp_data[i])){
	...
	if (vivo_otp_data[0x0A] != 0x11 ){
		OV13B10V1_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		ov13b10v1_flag = 0;
		LOG_INF("VCM ID Error!!!    ov13b10v1_flag =%d\n", ov13b10v1_flag);
	}else{
		ov13b10v1_flag = 1;
		LOG_INF("ov13b10v1_flag = %d\n", ov13b10v1_flag);
	}
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
        ...
        {
			OV13B10V1_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
            LOG_INF("ModuleInfo_checksum error!!!   otp_error_code:%d\n",OV13B10V1_OTP_ERROR_CODE);
			return 0;
        }
		/****material info start****/
			material_inf_main_ov13b10v1[0] = (MUINT32)vivo_otp_data[0x10];
			material_inf_main_ov13b10v1[1] = (MUINT32)vivo_otp_data[0x11];
			material_inf_main_ov13b10v1[2] = (MUINT32)vivo_otp_data[0x12];
			material_inf_main_ov13b10v1[3] = (MUINT32)vivo_otp_data[0x13];
			LOG_INF("material_inf_main_ov13b10v1[0] = 0x%x, material_inf_main_ov13b10v1[1] = 0x%x, material_inf_main_ov13b10v1[2] = 0x%x,material_inf_main_ov13b10v1[3] = 0x%x\n",
				material_inf_main_ov13b10v1[0]  , material_inf_main_ov13b10v1[1],  material_inf_main_ov13b10v1[2], material_inf_main_ov13b10v1[3]);
		/****material info end****/
		//////fuseIDInfo_checksum
		checksum = 0;
		...
		sn_inf_main_ov13b10v1[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_main_ov13b10v1[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_main_ov13b10v1[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_ov13b10v1[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//////AWB_checksum
		checksum = 0;
        ...
        if(vivo_otp_data[0x01] == 0x05){    
			...
			if (diff_inf_137 > 150 || diff_mac_137 > 150 || diff_inf_macro > 450 || diff_inf_macro < 150) {  /*AF code out of range*/
			    OV13B10V1_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("QTech 2311137 AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", OV13B10V1_OTP_ERROR_CODE, vivo_otp_data[0x0A], AF_inf, AF_mac);
			    return 0;
		    }
        }
		...
		OV13B10V1_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
