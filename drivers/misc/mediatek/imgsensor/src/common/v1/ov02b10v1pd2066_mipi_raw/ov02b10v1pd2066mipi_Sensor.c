...
#define PFX "OV02B10V1PD2066_camera_sensor"
#define LOG_1 LOG_INF("OV02B10V1PD2066,MIPI 1LANE\n")
/***************************   Modify end    *******************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B10V1PD2066_SENSOR_ID,
...
extern int ov02b10v1pd2066_otp_read(void);
extern otp_error_code_t OV02B10V1PD2066_OTP_ERROR_CODE;
MUINT32  sn_inf_main_ov02b10v1pd2066[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_ov02b10v1pd2066[4];  
MUINT32  module_inf_main_ov02b10v1pd2066[13];
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if ((*sensor_id + 1) == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = ov02b10v1pd2066_otp_read();
				printk("zhanghainan read eeprom ---vivo_otp_read_when_power_on = %d,OV02B10V1PD2066_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, OV02B10V1PD2066_OTP_ERROR_CODE);
				/*vivo lxd add end*/
#if 1
					printk("shine 2371271 module flag =%d module ID =%d \n", module_inf_main_ov02b10v1pd2066[0] , module_inf_main_ov02b10v1pd2066[1]);
				if (module_inf_main_ov02b10v1pd2066[0] == 1 && module_inf_main_ov02b10v1pd2066[1] == 0x04) {
					printk("shine 2371271 module flag =%d module ID =%d \n", module_inf_main_ov02b10v1pd2066[0] , module_inf_main_ov02b10v1pd2066[1]);
					return ERROR_NONE;
				}
#endif
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		...
		spin_lock(&imgsensor_drv_lock);
		imgsensor.current_fps = (UINT32)*feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
	    printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  OV02B10V1PD2066_OTP_ERROR_CODE:%d \n", *feature_data,OV02B10V1PD2066_OTP_ERROR_CODE);
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = OV02B10V1PD2066_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_ov02b10v1pd2066, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_ov02b10v1pd2066, sizeof(MUINT32)*4); 
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_ov02b10v1pd2066[%d]= 0x%x\n", i, sn_inf_main_ov02b10v1pd2066[i]);
						}
				#endif
			}
				break;
...
UINT32 OV02B10V1PD2066_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      OV02B10V1PD2066_MIPI_RAW_SensorInit      */
