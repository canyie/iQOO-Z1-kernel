...
#include "s5k3p9sp04pd1934v2mipiraw_Sensor.h"

#define PFX "S5K3P9SP04PD1934V2_EEPROM_OTP"
...
#endif
static int checksum;
otp_error_code_t S5K3P9SP04PD1934V2_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32 crosstalk_datav2[2049];  /*0 flag   1-606 data*/
extern MUINT32  sn_inf_sub_s5k3p9sp04pd1934v2[13];
extern MUINT32  material_inf_sub_s5k3p9sp04pd1934v2[4];
...
int S5K3P9SP04PD1934V2_otp_read(void)
{
	int i = 0;
	...
	/*if (is_atboot == 1) {
		LOG_INF("AT mode skip S5K3P9SP04PD1934V2_otp_read\n");
		return 1;
	}
	*/
	/* guojunzheng add end */
	...
	if ((0x01 == vivo_otp_data[ModuleInfo_addr]) && (0x01 == vivo_otp_data[Sensor_fuse_id_addr]) && (0x01 == vivo_otp_data[SN_addr]) && (0x01 == vivo_otp_data[Awb_addr]) && (0x01 == vivo_otp_data[Lsc_addr])&& (0x01 == vivo_otp_data[XTLK_addr])) {
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		if ((VIVO_VENDOR_TRULY != vivo_otp_data[0x01])&&(VIVO_VENDOR_OFILM != vivo_otp_data[0x01])){
			S5K3P9SP04PD1934V2_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("Module ID error!!!    otp_error_code:%d\n", S5K3P9SP04PD1934V2_OTP_ERROR_CODE);
			return 0;
		}
		...
		else if ((VIVO_VENDOR_LENS_ID != vivo_otp_data[0x08]) && (VIVO_VENDOR_VCM_ID != vivo_otp_data[0x09])&& (VIVO_VENDOR_DRIVERIC_ID != vivo_otp_data[0x0A])) {
			S5K3P9SP04PD1934V2_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		...
		if (vivo_otp_data[ModuleInfo_checksum_addr] != checksum) {
			S5K3P9SP04PD1934V2_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("ModuleInfo_checksum error!!!   otp_error_code:%d\n", S5K3P9SP04PD1934V2_OTP_ERROR_CODE);
			return 0;
			}
			for (i = 0; i < 2049; i++) {	
			crosstalk_datav2[i] = (MUINT32)vivo_otp_data[i + XTLK_addr];
			LOG_INF("crosstalk_data[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i, crosstalk_datav2[i],	i + XTLK_addr, vivo_otp_data[i + XTLK_addr]);
			}				
		...
		sn_inf_sub_s5k3p9sp04pd1934v2[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_sub_s5k3p9sp04pd1934v2[i +1] = (MUINT32)vivo_otp_data[i + SN_addr+1];
			/*LOG_INF("sn_inf_sub_s5k3p9sp04pd1934v2[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_sub_s5k3p9sp04pd1934v2[i +1],  i +SN_addr+1, vivo_otp_data[i + SN_addr+1]);*/
		}

        /*   ***material info start****/
        material_inf_sub_s5k3p9sp04pd1934v2[0] = (MUINT32)vivo_otp_data[0x0F];
        material_inf_sub_s5k3p9sp04pd1934v2[1] = (MUINT32)vivo_otp_data[0x10];
        material_inf_sub_s5k3p9sp04pd1934v2[2] = (MUINT32)vivo_otp_data[0x11];
        material_inf_sub_s5k3p9sp04pd1934v2[3] = (MUINT32)vivo_otp_data[0x12];
        LOG_INF("material_inf_sub_s5k3p9sp04pd1934v2[0] = 0x%x, material_inf_sub_s5k3p9sp04pd1934v2[1] = 0x%x, material_inf_sub_s5k3p9sp04pd1934v2[2] = 0x%x,material_inf_sub_s5k3p9sp04pd1934v2[3] = 0x%x\n",
            material_inf_sub_s5k3p9sp04pd1934v2[0],material_inf_sub_s5k3p9sp04pd1934v2[1],material_inf_sub_s5k3p9sp04pd1934v2[2],material_inf_sub_s5k3p9sp04pd1934v2[3]);
        /*   ***material info end****/
		...
		if (t > 0) {
			S5K3P9SP04PD1934V2_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, S5K3P9SP04PD1934V2_OTP_ERROR_CODE);
			return 0;
		}
		...
		if (diff_inf > 70 || diff_mac > 80 || diff_inf_macro > 450 || diff_inf_macro < 250) {  /*AF code out of range*/
			S5K3P9SP04PD1934V2_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
		...
		S5K3P9SP04PD1934V2_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
