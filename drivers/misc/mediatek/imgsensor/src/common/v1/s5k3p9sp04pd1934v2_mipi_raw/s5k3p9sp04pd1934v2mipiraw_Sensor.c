 ...
 * Filename:
 * ---------
 *	 S5K3P9SP04PD1934V2mipi_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#define PFX "S5K3P9SP04PD1934V2_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
		.sensor_id = S5K3P9SP04PD1934V2_SENSOR_ID,
...
extern int S5K3P9SP04PD1934V2_otp_read(void);
static int vivo_otp_read_when_power_on;
extern unsigned int is_atboot;
extern otp_error_code_t S5K3P9SP04PD1934V2_OTP_ERROR_CODE;
MUINT32  sn_inf_sub_s5k3p9sp04pd1934v2[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_sub_s5k3p9sp04pd1934v2[4];
MUINT32 crosstalk_datav2[2049];  /*0 flag   1-836 data*/
extern int read_sn_id_v2(void);
/****hope add end****/
			...
			*sensor_id = ((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001)) + 2;
			if (*sensor_id == imgsensor_info.sensor_id) {
             ...
             /*vivo hope add for AT cammand end*/
				...
				vivo_otp_read_when_power_on = S5K3P9SP04PD1934V2_otp_read();
				LOG_INF("cfx_add:end read eeprom ---vivo_otp_read_when_power_on = %d,S5K3P9SP04PD1934V2_OTP_ERROR_CODE =%d\n", vivo_otp_read_when_power_on, S5K3P9SP04PD1934V2_OTP_ERROR_CODE);
				/*vivo hope  add end*/
				return ERROR_NONE;
				} else {
				*sensor_id = 0xFFFFFFFF;
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5K3P9SP04PD1934V2_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_sub_s5k3p9sp04pd1934v2, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_sub_s5k3p9sp04pd1934v2, sizeof(MUINT32)*4);
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						LOG_INF("sn_inf_sub_s5k3p9sp04pd1934v2[%d]= 0x%x\n", i, sn_inf_sub_s5k3p9sp04pd1934v2[i]);
						}
				#endif
			}
				break;
...
UINT32 S5K3P9SP04PD1934V2_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!= NULL)
		*pfFunc= &sensor_func;
	return ERROR_NONE;
}	/*	S5K3P9SP04PD1934V2_MIPI_RAW_SensorInit	*/
