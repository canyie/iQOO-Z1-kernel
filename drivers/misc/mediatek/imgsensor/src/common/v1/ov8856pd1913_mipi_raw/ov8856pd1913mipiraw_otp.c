...
#include "ov8856pd1913mipiraw_Sensor.h"

#define PFX "OV8856PD1913_EEPROM_OTP"
...
static unsigned char main3_otp_data_ov8856pd1913[VIVO_MAIN3_OTP_DATA_SIZE];
...
#endif
static int checksum;
otp_error_code_t MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_main2_ov8856pd1913[13];
...
/*guojunzheng add*/
int vivo_main3_otp_read_ov8856pd1913(void)
{
	int i = 0;
	int offset = 0x0000;
	...
	MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_NORMAL;
	#if 1
	...
	for (i = 0; i < VIVO_MAIN3_OTP_DATA_SIZE; i++) {
		if (!vivo_read_eeprom(offset,  &main3_otp_data_ov8856pd1913[i])) {
			LOG_INF("read_vivo_eeprom 0x%0x %d fail \n", offset,  main3_otp_data_ov8856pd1913[i]);
			return 0;
		}
	...
	/*for(i = 0; i < VIVO_OTP_DATA_SIZE; i++) {
		LOG_INF("read_vivo_eeprom Data[0x%0x]:0x%x\n", i,  main3_otp_data_ov8856pd1913[i]);
	}*/

	/*check_if_group_valid = main3_otp_data_ov8856pd1913[ModuleInfo_addr] | main3_otp_data_ov8856pd1913[Awb_addr] | main3_otp_data_ov8856pd1913[Af_addr] | main3_otp_data_ov8856pd1913[Lsc_addr] | main3_otp_data_ov8856pd1913[PD_Proc1_addr] | main3_otp_data_ov8856pd1913[PD_Proc2_addr];*/
	if ((0x01 == main3_otp_data_ov8856pd1913[ModuleInfo_addr]) && (0x01 == main3_otp_data_ov8856pd1913[Fuse_id_addr]) && (0x01 == main3_otp_data_ov8856pd1913[SN_addr]) && (0x01 == main3_otp_data_ov8856pd1913[Awb_addr]) && (0x01 == main3_otp_data_ov8856pd1913[Lsc_addr])  ) {
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		/****ModuleInfo****/
		if (  (VIVO_MAIN3_VENDOR_TRULY != main3_otp_data_ov8856pd1913[0x01])) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("Module ID error!!!    otp_error_code:%d\n", MAIN3_OTP_ERROR_CODE_OV8856PD1913);
			return 0;
		} else if (((VIVO_MAIN3_VENDOR_LENS_ID != main3_otp_data_ov8856pd1913[0x0008])) || (VIVO_MAIN3_VENDOR_VCM_ID != main3_otp_data_ov8856pd1913[0x0009]) || (VIVO_MAIN3_VENDOR_DRIVERIC_ID != main3_otp_data_ov8856pd1913[0x000A]) || (VIVO_MAIN3_VENDOR_PLATFORM_ID != main3_otp_data_ov8856pd1913[0x0002])) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF(": Platform ID or Lens or VCM ID or Driver_IC ID  Error!!!    otp_error_code:%d\n", MAIN3_OTP_ERROR_CODE_OV8856PD1913);
			return 0;
		} else if ((0xff != main3_otp_data_ov8856pd1913[0x000B]) || (0x00 != main3_otp_data_ov8856pd1913[0x000C]) || (0x0b != main3_otp_data_ov8856pd1913[0x000D]) || (0x01 != main3_otp_data_ov8856pd1913[0x000E])) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF(": calibration version  Error!!!    Read version:0x%2x%2x%2x%2x\n", main3_otp_data_ov8856pd1913[0x000B], main3_otp_data_ov8856pd1913[0x000C], main3_otp_data_ov8856pd1913[0x000D], main3_otp_data_ov8856pd1913[0x000E]);
			return 0;
		}
		/****ModuleInfo_checksum****/
		checksum = 0;
		for (i = ModuleInfo_addr+1; i < ModuleInfo_checksum_addr; i++) {
			checksum += main3_otp_data_ov8856pd1913[i];
			}
			checksum = checksum % 0xff+1;
		if (main3_otp_data_ov8856pd1913[ModuleInfo_checksum_addr] != checksum) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Fuse_id_addr+1; i < Fuse_id_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (main3_otp_data_ov8856pd1913[Fuse_id_checksum_addr] != checksum) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Awb_addr+1; i < Awb_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (main3_otp_data_ov8856pd1913[Awb_checksum_addr] != checksum) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Af_addr+1; i < Af_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (main3_otp_data_ov8856pd1913[Af_checksum_addr] != checksum) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = Lsc_addr+1; i < Lsc_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (main3_otp_data_ov8856pd1913[Lsc_checksum_addr] != checksum) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = PDAF_proc1_addr+1; i < PDAF_proc1_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (main3_otp_data_ov8856pd1913[PDAF_proc1_checksum_addr] != checksum) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		for (i = PDAF_proc2_addr+1; i < PDAF_proc2_checksum_addr; i++) {
			...
			checksum = checksum % 0xff+1;
		if (main3_otp_data_ov8856pd1913[PDAF_proc2_checksum_addr] != checksum) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("PDAF_data_checksum error!!!   otp_error_code:%d\n", MAIN3_OTP_ERROR_CODE_OV8856PD1913);
			return 0;
		}
		...
		for (i = SN_addr+1; i < SN_checksum_addr; i++) {
			LOG_INF("main3_otp_data_ov8856pd1913[0x%x] = 0x%x\n", i, main3_otp_data_ov8856pd1913[i]);
			checksum += main3_otp_data_ov8856pd1913[i];
			}
			checksum = checksum % 0xff+1;
		if (main3_otp_data_ov8856pd1913[SN_checksum_addr] != checksum) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("SN_checksum error!!!   otp_error_code:%d\n", MAIN3_OTP_ERROR_CODE_OV8856PD1913);
			return 0;
		}
		sn_inf_main2_ov8856pd1913[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_main2_ov8856pd1913[i +1] = (MUINT32)main3_otp_data_ov8856pd1913[i + SN_addr+1];
			/*LOG_INF("sn_inf_main2_ov8856pd1913[%d] = 0x%x, main3_otp_data_ov8856pd1913[0x%x] = 0x%x\n", i+1  , sn_inf_main2_ov8856pd1913[i +1],  i +SN_addr+1, main3_otp_data_ov8856pd1913[i + SN_addr+1]);*/
		}


		/****check if awb out of range[high cct]****/
		R_unit = main3_otp_data_ov8856pd1913[Awb_addr+1];
		R_unit = (R_unit << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+2]);
		B_unit = main3_otp_data_ov8856pd1913[Awb_addr+3];
		B_unit = (B_unit << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+4]);
		G_unit = main3_otp_data_ov8856pd1913[Awb_addr+5];
		G_unit = (G_unit << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+6]);

		R_golden = main3_otp_data_ov8856pd1913[Awb_addr+7];
		R_golden = (R_golden << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+8]);
		B_golden = main3_otp_data_ov8856pd1913[Awb_addr+9];
		B_golden = (B_golden << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+10]);
		G_golden = main3_otp_data_ov8856pd1913[Awb_addr+11];
		G_golden = (G_golden << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+12]);
		...
		if (t > 0) {
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, MAIN3_OTP_ERROR_CODE_OV8856PD1913);
			return 0;
		}
		/****check if awb out of range[low cct]****/
		R_unit_low = main3_otp_data_ov8856pd1913[Awb_addr+13];
		R_unit_low = (R_unit_low << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+14]);
		B_unit_low = main3_otp_data_ov8856pd1913[Awb_addr+15];
		B_unit_low = (B_unit_low << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+16]);
		G_unit_low = main3_otp_data_ov8856pd1913[Awb_addr+17];
		G_unit_low = (G_unit_low << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+18]);

		R_golden_low = main3_otp_data_ov8856pd1913[Awb_addr+19];
		R_golden_low = (R_golden_low << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+20]);
		B_golden_low = main3_otp_data_ov8856pd1913[Awb_addr+21];
		B_golden_low = (B_golden_low << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+22]);
		G_golden_low = main3_otp_data_ov8856pd1913[Awb_addr+23];
		G_golden_low = (G_golden_low << 8) | (main3_otp_data_ov8856pd1913[Awb_addr+24]);
		...
		AF_inf  =  main3_otp_data_ov8856pd1913[Af_addr+2];
		AF_inf = (AF_inf << 8) | (main3_otp_data_ov8856pd1913[Af_addr+3]);

		AF_mac = main3_otp_data_ov8856pd1913[Af_addr+4];
		AF_mac = (AF_mac << 8) | (main3_otp_data_ov8856pd1913[Af_addr+5]);
		...
		if (diff_inf > 70 || diff_mac > 80 || diff_inf_macro > 450 || diff_inf_macro < 250) {  /*AF code out of range*/
			MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
		...
		/*for(i = 0;i < 1372;i++)
		{
			...
			if(i < 496)
				PDAF_data[i] = main3_otp_data_ov8856pd1913[PD_Proc1_addr+i+1];
			else //i >= 496
				PDAF_data[i] = main3_otp_data_ov8856pd1913[PD_Proc1_addr+i+3];
		}*/
		/*copy pdaf data*/
		/*memcpy(PDAF_data, &main3_otp_data_ov8856pd1913[PDAF_addr+1], PDAF_checksum_addr-PDAF_addr-1);*/
		/*cfx add for pdaf data end*/
		MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_NORMAL;
		return 1;
	} else {
		MAIN3_OTP_ERROR_CODE_OV8856PD1913 = OTP_ERROR_CODE_GROUP_INVALID;
