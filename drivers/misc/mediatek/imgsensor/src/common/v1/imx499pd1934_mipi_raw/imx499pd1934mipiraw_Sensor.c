 ...
 *=====================================================
 ************************************************************************/
#define PFX "IMX499PD1934_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX499PD1934_SENSOR_ID,	/* record sensor id defined in Kd_imgsensor.h */
...
extern int imx499pd1934_otp_read(void);
extern otp_error_code_t IMX499PD1934_OTP_ERROR_CODE;

MUINT32  sn_inf_main_imx499pd1934[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_imx499pd1934[4];
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	pr_debug("imx499pd1934 gain =%d, reg_gain =%d\n", gain, reg_gain);
	return reg_gain;
}
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if ((*sensor_id == imgsensor_info.sensor_id)||((*sensor_id + 1) == imgsensor_info.sensor_id)) {
				...
				vivo_otp_read_when_power_on = imx499pd1934_otp_read();
				pr_debug("lxd_add:end read eeprom ---vivo_otp_read_when_power_on = %d,IMX499PD1934_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, IMX499PD1934_OTP_ERROR_CODE);
				/*vivo lxd add end*/
				return ERROR_NONE;
			}
...
static kal_uint32 imx499pd1934_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_AWB_GAIN:
		imx499pd1934_awb_gain(pSetSensorAWB);
		break;
	...
	case SENSOR_FEATURE_SET_PDAF:
		...
		case SENSOR_FEATURE_GET_CUSTOM_INFO:
			#if 1
		    pr_debug("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  IMX499PD1934_OTP_ERROR_CODE:%d \n", *feature_data,IMX499PD1934_OTP_ERROR_CODE);
			switch (*feature_data) {
				...
				if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
				    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = IMX499PD1934_OTP_ERROR_CODE;//otp_state
					memcpy(feature_data+2, sn_inf_main_imx499pd1934, sizeof(MUINT32)*13); 
					memcpy( feature_data+10, material_inf_main_imx499pd1934, sizeof(MUINT32)*4);
					#if 0
							for (i = 0 ; i<13 ; i++ ){
							printk("sn_inf_main_imx499pd1934[%d]= 0x%x\n", i, sn_inf_main_imx499pd1934[i]);
							}
...
UINT32 IMX499PD1934_MIPI_RAW_SensorInit(
			struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*    IMX499PD1934_MIPI_RAW_SensorInit    */
