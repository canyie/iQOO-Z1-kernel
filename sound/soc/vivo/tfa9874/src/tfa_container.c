 ...
 */
TfaLiveDataList_t *tfaContGetDevLiveDataList(TfaContainer_t *cont, int devIdx, int lifeDataIdx)
{
	TfaDeviceList_t *dev;
	int idx, hit;
	uint8_t *base = (uint8_t *)cont;
...
enum Tfa98xx_Error tfaContWriteRegsDev(struct tfa_device *tfa)
{
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscPatch || dev->list[i].type == dscFile || dev->list[i].type == dscProfile)
			break;
