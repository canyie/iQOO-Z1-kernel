...
tfa98xx_wait_result(struct tfa_device *tfa, int wait_retry_count)
{
	...
	while ((error == Tfa98xx_Error_Ok) && ((cf_status & CF_STATUS_I2C_CMD_ACK) == 0) && (tries < wait_retry_count))
		;
					...
					unsigned char data[])
{
	...
	error = dsp_msg_read(tfa, read_length, data); 
	return error; 
}
...
void tfa_adapt_noisemode(struct tfa_device *tfa)
{
	...
	if (avbatt <= MIN_BATT_LEVEL && !tfa_get_bf(tfa, TFA9873_BF_LNMODE)) {
		//640 corresponds to 3.4 volt, MCH_TO_TEST
