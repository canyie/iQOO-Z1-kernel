...
#include "../inc/tfa9894-debug-common.h"
...
static char *default_fw_name = "tfa98xx.cnt";
static char *fw_name = "tfa98xx.cnt";
static char cnt_name[48];
...
static const char *mute_function[] = {"ON", "OFF"};
	...
	if (tfa98xx_priv->tfa->is_probus_device) {
		...
		list_all_tfa98xx (tfa98xx) {
			...
			n += scnprintf (buffer + n, size - n, 
						"current status: mono: impedance %d.%02d ohm, valid range (%d.%02d ~ %d.%02d ohm). \n",
						course, fine, 
...
int tfa98xx_check_mtp_dbg(void)
{
	...
	list_all_tfa98xx (tfa98xx) {
		...
		if (TFA_GET_BF(tfa, MTPEX) == 1) {
			...
			ret |= tfa98xx_calibration_done (value, tfa98xx) << i;
			pr_info("%s: impadence: %d, ret=%d\n", __func__, value, ret);
		}
		++i;
	}
...
int tfa98xx_read_freq_dbg(char *buffer, int size)
{
	...
	for (i = 0 ; i < channel_count; i++) {
		...
		while (((count_i[i]++ < 5) && (count_j[i] < 3))) {
			...
			n += scnprintf(buffer+n, size-n, "f0 = %d Qt = %d.%03d\n", f0_data[i], q_value[i]/1024, (q_value[i]*1000/1024)%1000);
		...
		n += scnprintf(buffer+n, size-n, "f0 (%d ~ %d)\nQt_Min: %d.%02d\n",
				   ...
				   tfa98xx->para.Qt/100, tfa98xx->para.Qt%100);
		if (count_j[i] == 3)
				...
				loff_t *ppos)
{
	...
	struct tfa98xx *tfa98xx = (struct tfa98xx *)file->private_data;
	char *str;
	...
	if (!tfa98xx) {
		tfa98xx = tfa98xx_priv;
		pr_err("get NULL *data, use tfa98xx_priv!\n");
	}
...
{
	struct tfa98xx *tfa98xx = (struct tfa98xx *)data;
	int value;
 ...
 */
static ssize_t smartpa_otc_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	int value = 0, n = 0, size = 512;
	...
	pr_debug("[0x%x] OTC : %d\n", tfa98xx->i2c->addr, value);
	n += scnprintf(ubuf+n, size-n, "%x\n", value);
	ubuf[n] = 0;

	return n;
}

static ssize_t smartpa_otc_store(struct kobject *kobj, struct kobj_attribute *attr,
...
static ssize_t smartpa_mtpex_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
	 struct tfa98xx *tfa98xx = tfa98xx_priv;
	 int value = 0, n = 0, size = 512;
...
static ssize_t smartpa_mtpex_store(struct kobject *kobj, struct kobj_attribute *attr,
			   const char *ubuf, size_t count)
{
	 struct tfa98xx *tfa98xx = tfa98xx_priv;
	 enum Tfa98xx_Error err;
...
static ssize_t smartpa_temp_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
   struct tfa98xx *tfa98xx = tfa98xx_priv;
   int n = 0, val = 0, size = 512;
...
static ssize_t smartpa_temp_store(struct kobject *kobj, struct kobj_attribute *attr,
			 const char *ubuf, size_t count)
{
   struct tfa98xx *tfa98xx = tfa98xx_priv;
   unsigned int kbuf;
   ...
   mutex_lock(&tfa98xx->dsp_lock);
   tfa98xx_set_exttemp(tfa98xx->tfa, kbuf);
   mutex_unlock(&tfa98xx->dsp_lock);
   pr_debug("[0x%x] TEMP < %llu\n", tfa98xx->i2c->addr, kbuf);
   kfree(temp);

   return count;
}

static ssize_t smartpa_calibrate_store(struct kobject *kobj, struct kobj_attribute *attr,
		const char *ubuf, size_t count)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	enum Tfa98xx_Error ret;
...
static ssize_t smartpa_r_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	char *str;
	uint16_t status;
	int ret = 0, n = 0, size = 512;
...
static ssize_t smartpa_version_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
	char str[] = TFA98XX_VERSION "\n";
	int n = 0, size = 512;
...
static ssize_t smartpa_dsp_state_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	int n = 0, size = 512;
	char *str;
...
static ssize_t smartpa_fw_state_store(struct kobject *kobj, struct kobj_attribute *attr,
			  const char *ubuf, size_t count)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	enum Tfa98xx_Error ret;
...
static ssize_t smartpa_fw_state_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
  struct tfa98xx *tfa98xx = tfa98xx_priv;
	char *str;
  int n = 0, size = 512;
...
static ssize_t smartpa_rpc_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	int ret = 0;
	size_t count = 512;
	...
	ret = copy_to_user(ubuf, buffer, count);
	kfree(buffer);
	if (ret)
		return -EFAULT;
...
static ssize_t smartpa_rpc_store(struct kobject *kobj, struct kobj_attribute *attr,
		const char *ubuf, size_t count)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	...
	if (copy_from_user(msg_file->data, ubuf, count))
		return -EFAULT;
...
static ssize_t smartpa_pga_gain_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	unsigned int value, n = 0, size = 512;
...
static ssize_t smartpa_pga_gain_store(struct kobject *kobj, struct kobj_attribute *attr,
			  const char *ubuf, size_t count)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	uint16_t value;
...
static struct kobj_attribute dev_attr_otc =
	__ATTR(OTC, 0664, smartpa_otc_show, smartpa_otc_store);
static struct kobj_attribute dev_attr_mtpex =
	__ATTR(MTPEX, 0664, smartpa_mtpex_show, smartpa_mtpex_store);
static struct kobj_attribute dev_attr_temp =
	__ATTR(TEMP, 0664, smartpa_temp_show, smartpa_temp_store);
static struct kobj_attribute dev_attr_calibrate =
	__ATTR(calibrate, 0664, NULL, smartpa_calibrate_store);
static struct kobj_attribute dev_attr_r =
	__ATTR(R, 0664, smartpa_r_show, NULL);
static struct kobj_attribute dev_attr_version =
	__ATTR(version, 0664, smartpa_version_show, NULL);
static struct kobj_attribute dev_attr_dsp_state =
	__ATTR(dsp-state, 0664, smartpa_dsp_state_show, NULL);
static struct kobj_attribute dev_attr_fw_state =
	__ATTR(fw-state, 0664, smartpa_fw_state_show, smartpa_fw_state_store);
static struct kobj_attribute dev_attr_rpc =
	__ATTR(rpc, 0664, smartpa_rpc_show, smartpa_rpc_store);
static struct kobj_attribute dev_attr_i2c =
	__ATTR(i2c, 0664, smartpa_i2c_show, NULL);
static struct kobj_attribute dev_attr_pga_gain =
	__ATTR(pga_gain, 0664, smartpa_pga_gain_show, smartpa_pga_gain_store);
...
static const struct soc_enum tfa98xx_snd_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mute_function), mute_function),
};

static int tfa98xx_get_ctl_mute(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct tfa98xx *tfa98xx = NULL;

	mutex_lock(&tfa98xx_mutex);
	tfa98xx = list_first_entry(&tfa98xx_device_list, typeof(*tfa98xx), list);
	...
	pr_info("%s : get tfa98xx's mute_state(%d)\n", __func__, tfa98xx->mute_state);
...
static int tfa98xx_set_ctl_mute(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct tfa98xx *tfa98xx = NULL;
	int mute = 0;
	...
	if (tfa98xx == NULL) {
		pr_err("%s : tfa98xx is NULL, can not set mute state!\n", __func__);
		mutex_unlock(&tfa98xx_mutex);
		return -1;
	}
	...
	// mute=0 is open, mute=1 is close
	mute = ucontrol->value.integer.value[0];
	if ((mute != 0) && (mute != 1)) {
		pr_err("%s : mute value is not invalid, can not set mute state!\n", __func__);
		return -1;
	}

	if (no_start) {
		pr_err("%s : no_start parameter set no tfa_dev_start or tfa_dev_stop, returning\n", __func__);
		return 0;
	}

	pr_info("%s : tfa98xx->calibration=%d, tfa98xx->tfa->is_probus_device=%d\n", __func__,
			tfa98xx->calibration, tfa98xx->tfa->is_probus_device);
	if ((!tfa98xx->calibration) && (!tfa98xx->tfa->is_probus_device)) {
		pr_err("%s : SmartPA Calibration not finish or failed. Not open.\n", __func__);
		return 0;
	}

	pr_info("%s : set tfa98xx's mute_state(%d)\n", __func__, mute);
	if (mute) {
		...
		cancel_delayed_work_sync(&tfa98xx->init_work);
		if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
			pr_info("%s : tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK, can not close smartpa and DSP\n", __func__);
			return 0;
		}
		...
		if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
			if (0)
				tfa98xx_send_mute_cmd(); //qinbaoqiang remove for pa off in mute playback
			msleep(60);
		}
...
#endif
		/* Start DSP */
		if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_PENDING && tfa98xx->calibration == 1)
			tfa98xx_dsp_init(tfa98xx);

		if (tfa98xx->flags & TFA98XX_FLAG_ADAPT_NOISE_MODE)
			queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->nmodeupdate_work, 0);

		if (tfa98xx->calibration == 1) {
			mutex_lock(&tfa98xx_mutex);
			tfa98xx->mute_state = mute;
			pr_info("%s : set tfa98xx's mute_state(%d)\n", __func__, tfa98xx->mute_state);
			mutex_unlock(&tfa98xx_mutex);
		} else
			pr_err("%s : can not set tfa98xx's mute_state(tfa98xx->calibration != 1)\n", __func__);
	}
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	/* Create a mute control */
	name = devm_kzalloc(tfa98xx->codec->dev, MAX_CONTROL_NAME, GFP_KERNEL);
	if (!name)
		return -ENOMEM;

	scnprintf(name, MAX_CONTROL_NAME, "tfa98xx_mute_ctrl");
	tfa98xx_controls[mix_index].name = name;
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
    ...
    tfa98xx_controls[mix_index].private_value = (unsigned long)&tfa98xx_snd_enum[0];
	mix_index++;
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values(void)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if (TFA_GET_BF(tfa, MTPEX) == 1) {
			...
			if (value < tfa98xx->para.imped_min || value > tfa98xx->para.imped_max) {
				/*mark as not power up IC*/
				tfa98xx->calibration = 0;
			} else {
				/*mark as good, we can power up IC*/
				tfa98xx->calibration = 1;
			}
		}
	}
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		// stop DSP only when both playback and capture streams are deactivated
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx, struct device_node *np)
{
	...
	tfa98xx->para.Qt = (!ret) ? (int)temp:125;
	...
	const struct i2c_device_id *id)
{
	...
	class_attr_create(tfa98xx->kobj, i2c);
	tfa98xx->mute_state = 1;
    tfa98xx->calibration = 1;
...
static int tfa98xx_i2c_remove(struct i2c_client *i2c)
{
	...
	class_attr_remove(tfa98xx->kobj);
