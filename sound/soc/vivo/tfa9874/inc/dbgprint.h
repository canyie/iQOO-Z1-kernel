...
#   define PRINT_ASSERT(e) do { if ((e)) printk(KERN_ERR "PrintAssert:%s (%s:%d) error code:%d\n", __FUNCTION__, __FILE__, __LINE__, e); } while (0)
...
#	define PRINT_ASSERT(e) do { if ((e)) fprintf(stderr, "PrintAssert:%s (%s:%d) error code:%d\n", __FUNCTION__, __FILE__, __LINE__, e); } while (0)
