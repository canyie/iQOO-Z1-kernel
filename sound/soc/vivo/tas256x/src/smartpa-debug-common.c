...
extern int smartpa_init_dbg_v1(char *buffer, int size);
extern int smartpa_read_freq_dbg_v1(char *buffer, int size);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case TFA_IOCTL_SPK_INTS:
		...
		check = smartpa_init_dbg_v1(msg.msgs, MSGS_SIZE);
	...
	case TFA_IOCTL_SPK_RFDES:
		...
		ret = smartpa_read_freq_dbg_v1(msg.msgs, MSGS_SIZE);
		ret = copy_to_user((void *)arg, &msg, sizeof(struct smartpa_msg));
		break;
		/* checkmtp */
