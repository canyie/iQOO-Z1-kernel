...
static int tas256x_mute_stream(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (mute)
			tas256x_set_power_state(p_tas256x, TAS256X_POWER_SHUTDOWN);
		else
...
static int tas256x_codec_probe(struct snd_soc_component *codec)
{
	...
	printk("%s: enter tas256x_codec_probe ===>\n", __func__);
	...
	/* Generic Probe */
	printk("%s: enter tas256x_probe ===>\n", __func__);
	ret = tas256x_probe(p_tas256x);
	printk("%s: leave tas256x_probe <===\n", __func__);
#ifdef CONFIG_TAS25XX_ALGO
	#ifdef CONFIG_TAS256x_FOR_MTK
	tas_smartamp_add_algo_controls_debugfs(codec, p_tas256x->mn_channels);
	#else
	tas_smartamp_add_algo_controls(codec, p_tas256x->mn_channels);
	#endif
#endif
	dev_info(plat_data->dev, "%s\n", __func__);

	printk("%s: leave tas256x_codec_probe <===\n", __func__);
...
int tas256x_register_codec(struct tas256x_priv *p_tas256x)
{
	...
	dev_info(plat_data->dev, "%s, enter (%s) \n", __func__, dev_name(plat_data->dev));
	...
	vivo_set_codec_name(dev_name(plat_data->dev));
