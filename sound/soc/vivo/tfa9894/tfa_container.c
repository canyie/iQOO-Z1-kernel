...
static char *tfaContGetString(nxpTfaContainer_t *cnt, nxpTfaDescPtr_t *dsc);
static char *tfaContGetCommandString(uint32_t type);
static int tfaContCrcCheckContainer(nxpTfaContainer_t *cont);
static nxpTfaFileDsc_t *tfacont_getfiledata(struct tfa_device *tfa, int prof_idx, enum nxpTfaHeaderType type);
...
*/
enum Tfa98xx_Error tfa_load_cnt_v1(void *cnt,  int length) 
{
 ...
 */
static void tfaContShowHeader(nxpTfaHeader_t *hdr) 
{
	char _id[2];
 ...
 */
static nxpTfaDeviceList_t *tfaContGetDevList(nxpTfaContainer_t *cont,  int dev_idx)
{
 ...
 */
static nxpTfaProfileList_t *tfaContGetDevProfList(nxpTfaContainer_t *cont,  int devIdx,  int profIdx)
{
 ...
 */
/*static nxpTfaLiveDataList_t *tfaContGetDevLiveDataList(nxpTfaContainer_t *cont,  int devIdx,  
					   int lifeDataIdx)
{
	nxpTfaDeviceList_t *dev;
	int idx,  hit;
 ...
 */
int tfacont_get_max_vstep_v1(struct tfa_device *tfa,  int prof_idx) 
{
...
static enum Tfa98xx_Error tfaContWriteVstep(struct tfa_device *tfa,   nxpTfaVolumeStep2File_t *vp,  int vstep)
{
	...
	if (vstep < vp->vsteps) {
		...
		err = tfa98xx_set_volume_level_v1(tfa,  vol); 
		if (err != Tfa98xx_Error_Ok)
			return err; 

		err = tfa98xx_dsp_write_preset_v1(tfa,  sizeof(vp->vstep[0].preset), vp->vstep[vstep].preset); 
		if (err != Tfa98xx_Error_Ok)
			return err; 
		err = tfa_cont_write_filterbank_v1(tfa,  vp->vstep[vstep].filter); 
...
static enum Tfa98xx_Error tfaContWriteVstepMax2(struct tfa_device *tfa,  nxpTfaVolumeStepMax2File_t *vp,  int vstep_idx,  int vstep_msg_idx)
{
	...
	for (i = 0;  i < regInfo->NrOfRegisters*2;  i++) {
		...
		err = tfaRunWriteBitfield_v1 (tfa,  bitF); 
		if (err != Tfa98xx_Error_Ok)
			return err; 
	}
...
enum Tfa98xx_Error tfaContWriteDrcFile_v1(struct tfa_device *tfa,  int size,  uint8_t data[])
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	uint8_t *msg = NULL; 
 ...
 */
enum Tfa98xx_Error tfaContWriteFile_v1(struct tfa_device *tfa,   nxpTfaFileDsc_t *file,  int vstep_idx,  int vstep_msg_idx)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	...
	case speakerHdr:  
		if (tfa->tfa_family == 2) {
			...
			err = tfa98xx_dsp_write_speaker_parameters_v1 (tfa,  size,  
					 (const unsigned char *) ((nxpTfaSpeakerFile_t *)hdr)->data);  
		}
		break;  
	case presetHdr:  
		size = hdr->size - sizeof (nxpTfaPreset_t);  
		err = tfa98xx_dsp_write_preset_v1 (tfa,  size,  (const unsigned char *) ((nxpTfaPreset_t *)hdr)->data);  
		break;  
	case equalizerHdr:  
		err = tfa_cont_write_filterbank_v1 (tfa,  ((nxpTfaEqualizerFile_t *)hdr)->filter);  
		break;  
	case patchHdr:  
		size = hdr->size - sizeof (nxpTfaPatch_t);   /* size is total length */
		err = tfa_dsp_patch_v1(tfa,   size,  (const unsigned char *) ((nxpTfaPatch_t *)hdr)->data);  
		break;  
	case configHdr:  
		size = hdr->size - sizeof (nxpTfaConfig_t);  
		err = tfa98xx_dsp_write_config_v1 (tfa,  size,  (const unsigned char *) ((nxpTfaConfig_t *)hdr)->data);  
		break;  
	case drcHdr:  
		if (hdr->version[0] == NXPTFA_DR3_VERSION) {
			/* Size is total size - hdrsize (36) - xmlversion (3) */
			size = hdr->size - sizeof (nxpTfaDrc2_t);  
			err = tfaContWriteDrcFile_v1 (tfa,  size,  ((nxpTfaDrc2_t *)hdr)->data);  
		} else {
			/*
			...
			err = tfa98xx_dsp_write_drc_v1 (tfa,  size,  ((const unsigned char *) ((nxpTfaDrc_t *)hdr)->data+381));  
		}
		break;  
	case infoHdr:  
		/* Ignore */
...
{
	nxpTfaDeviceList_t *dev = tfaContDevice_v1 (cnt,  dev_idx);  
 ...
 */
static int tfa_cnt_get_devid(nxpTfaContainer_t *cnt,  int dev_idx) 
{
 ...
 */
enum Tfa98xx_Error tfaContGetSlave_v1(struct tfa_device *tfa,  uint8_t *slave_addr)
{
	nxpTfaDeviceList_t *dev = NULL; 

	/* Make sure the cnt file is loaded */
	if (tfa->cnt != NULL) {
		dev = tfaContDevice_v1 (tfa->cnt,  tfa->dev_idx); 
	}
...
int tfa_cont_get_idx(struct tfa_device *tfa)
{
	...
	for (i = 0; i < tfa->cnt->ndev; i++) {
		dev = tfaContDevice_v1 (tfa->cnt,  i); 
 ...
 * write a bit field
 */
enum Tfa98xx_Error tfaRunWriteBitfield_v1(struct tfa_device *tfa,   nxpTfaBitfield_t bf) 
{
...
#endif
		error = tfa_set_bf_v1 (tfa,  bfUni.field,  value); 
 ...
 * read a bit field
 */
enum Tfa98xx_Error tfaRunReadBitfield_v1(struct tfa_device *tfa,   nxpTfaBitfield_t *bf) 
{
	enum Tfa98xx_Error error; 
	union {
		uint16_t field; 
...
static enum Tfa98xx_Error tfaRunWriteFilter(struct tfa_device *tfa,  nxpTfaContBiquad_t *bq) 
{
	...
	for (i = 0; i < runs; i++) {
		...
		dmem = tfa98xx_filter_mem_v1 (tfa,  bq->aa.index,  &address,  channel); 
		...
		if (tfa->tfa_family == 2) {
			error = tfa_dsp_cmd_id_write_v1 (tfa,  MODULE_FRAMEWORK,  FW_PAR_ID_SET_MEMORY,  sizeof (data), data); 
		} else {
			error = tfa_dsp_cmd_id_write_v1 (tfa,  MODULE_FRAMEWORK,  4 /* param */,  sizeof (data), data); 
		}
	}
...
enum Tfa98xx_Error tfaContWriteRegsDev_v1(struct tfa_device *tfa) 
{
...
enum Tfa98xx_Error tfaContWriteRegsProf_v1(struct tfa_device *tfa,  int prof_idx) 
{
...
enum Tfa98xx_Error tfaContWritePatch_v1(struct tfa_device *tfa) 
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	nxpTfaDeviceList_t *dev = tfaContDevice_v1 (tfa->cnt,  tfa->dev_idx); 
	nxpTfaFileDsc_t *file; 
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscPatch) {
			...
			err = tfa_dsp_patch_v1 (tfa,  size,  (const unsigned char *) patchfile->data); 
			if (err) 
				return err; 
		}
	}
...
enum Tfa98xx_Error tfaContWriteFiles_v1(struct tfa_device *tfa) 
{
	nxpTfaDeviceList_t *dev = tfaContDevice_v1 (tfa->cnt,  tfa->dev_idx); 
	nxpTfaFileDsc_t *file; 
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscFile) {
			file = (nxpTfaFileDsc_t *) (dev->list[i].offset + (uint8_t *)tfa->cnt); 
			if (tfaContWriteFile_v1 (tfa,   file,  0,  TFA_MAX_VSTEP_MSG_MARKER)) {
				return Tfa98xx_Error_Bad_Parameter; 
			}
		}
 ...
 */
enum Tfa98xx_Error tfaContWriteFilesProf_v1(struct tfa_device *tfa,  int prof_idx,  int vstep_idx) 
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	nxpTfaProfileList_t *prof = tfaContGetDevProfList (tfa->cnt,  tfa->dev_idx,  prof_idx); 
	char buffer[(MEMTRACK_MAX_WORDS * 3) + 3] = {0}; /*every word requires 3 and 3 is the msg */
	...
	for (i = 0; i < prof->length; i++) {
		...
		case dscFile: 
			file = (nxpTfaFileDsc_t *) (prof->list[i].offset + (uint8_t *)tfa->cnt); 
			err = tfaContWriteFile_v1 (tfa,   file,  vstep_idx,  TFA_MAX_VSTEP_MSG_MARKER); 
			break; 
		case dscPatch: 
			file = (nxpTfaFileDsc_t *) (prof->list[i].offset + (uint8_t *)tfa->cnt); 
			patchfile = (nxpTfaPatch_t *)&file->data; 
...
static enum Tfa98xx_Error tfaContWriteItem(struct tfa_device *tfa,  nxpTfaDescPtr_t *dsc)
{
	...
	case dscMode:
		...
		if (cas->value == Tfa98xx_Mode_RCV)
			tfa98xx_select_mode_v1(tfa, Tfa98xx_Mode_RCV);
		else
			tfa98xx_select_mode_v1 (tfa,  Tfa98xx_Mode_Normal); 
		break; 
	case dscCfMem: 
	...
	case dscBitfield: 
		...
		return tfaRunWriteBitfield_v1 (tfa,  *bitf); 
	break; 
	case dscFilter: 
...
#if 0
static enum Tfa98xx_Error tfa_write_filters(struct tfa_device *tfa,  int prof_idx) 
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	nxpTfaProfileList_t *prof = tfaContGetDevProfList (tfa->cnt,  tfa->dev_idx,  prof_idx); 
	unsigned int i; 
...
#endif
unsigned int tfa98xx_get_profile_sr_v1(struct tfa_device *tfa,  unsigned int prof_idx)
{
	nxpTfaBitfield_t *bitf; 
	unsigned int i; 
	nxpTfaDeviceList_t *dev; 
...
static enum Tfa98xx_Error get_sample_rate_info(struct tfa_device *tfa,  nxpTfaProfileList_t *prof,  nxpTfaProfileList_t *previous_prof,  int fs_previous_profile)
{
	...
	*/
	...
	if (fs_next_profile != fs_default_profile) {
		 ...
		 */
		err = tfa98xx_dsp_write_tables_v1 (tfa,  fs_next_profile); 
		 ...
		 */
		err = tfa98xx_dsp_write_tables_v1 (tfa,  fs_default_profile); 
	}
 ...
 *
 */
enum Tfa98xx_Error tfaContWriteProfile_v1(struct tfa_device *tfa,  int prof_idx,  int vstep_idx) 
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	nxpTfaProfileList_t *prof = tfaContGetDevProfList (tfa->cnt,  tfa->dev_idx,  prof_idx); 
	...
	if (tfa->verbose) {
		tfa98xx_trace_printk("device:%s profile:%s vstep:%d\n", tfaContDeviceName_v1(tfa->cnt, tfa->dev_idx),
					tfaContProfileName_v1(tfa->cnt, tfa->dev_idx, prof_idx), vstep_idx);
	}
 ...
 *
 */
enum Tfa98xx_Error tfaContWriteFilesVstep_v1(struct tfa_device *tfa,  int prof_idx,  int vstep_idx)
{
	nxpTfaProfileList_t *prof = tfaContGetDevProfList (tfa->cnt,  tfa->dev_idx,  prof_idx); 
	unsigned int i; 
	nxpTfaFileDsc_t *file; 
	...
	for (i = 0; i < prof->length; i++) {
		if (prof->list[i].type == dscFile) {
			...
			switch (type) {
			case volstepHdr: 
				if (tfaContWriteFile_v1 (tfa,  file,  vstep_idx,  TFA_MAX_VSTEP_MSG_MARKER))
					return Tfa98xx_Error_Bad_Parameter; 
				break; 
			default: 
				break; 
 ...
 *  return device name
 */
char  *tfaContDeviceName_v1(nxpTfaContainer_t *cnt,  int dev_idx)
{
	nxpTfaDeviceList_t *dev; 

	dev = tfaContDevice_v1(cnt,  dev_idx); 
	if (dev == NULL)
		return "!ERROR!"; 
 ...
 *
 */
int tfa_cnt_get_app_name_v1(struct tfa_device *tfa,  char *name)
{
	unsigned int i; 
	int len = 0; 
 ...
 */
int tfaContGetCalProfile_v1(struct tfa_device *tfa) 
{
	int prof,  cal_idx = -2; 
	int ret = 0; 

	pr_info("%s enter.\n", __func__);
	if ((tfa->dev_idx < 0) || (tfa->cnt && (tfa->dev_idx >= tfa->cnt->ndev))
		|| (tfa->cnt == NULL)) {
		ret = -1; 
		pr_err("%s tfa98xx devices error.\n", __func__);
		return ret; 
	}

	/* search for the calibration profile in the list of profiles */
	for (prof = 0; prof < tfa->cnt->nprof; prof++) {
		if (strnstr(tfaContProfileName_v1(tfa->cnt,  tfa->dev_idx,  prof),
			".cal",
			strlen(tfaContProfileName_v1 (tfa->cnt,  tfa->dev_idx,  prof))) != NULL) {
			cal_idx = prof; 
			pr_debug ("Using calibration profile; '%s'\n", tfaContProfileName_v1 (tfa->cnt,  tfa->dev_idx,  prof)); 
			break; 
		}
	}
 ...
 */
static int tfaContIsTapProfile(struct tfa_device *tfa,  int prof_idx)
{
	int ret = 0; 
	...
	if (strnstr(tfaContProfileName_v1(tfa->cnt, tfa->dev_idx, prof_idx), ".tap", strlen(tfaContProfileName_v1(tfa->cnt, tfa->dev_idx, prof_idx))) != NULL) {
		pr_debug("Using Tap profile: '%s'\n", tfaContProfileName_v1(tfa->cnt, tfa->dev_idx, prof_idx));
		return 1;
	}
 ...
 *  return profile name
 */
char *tfaContProfileName_v1(nxpTfaContainer_t *cnt,  int dev_idx,  int prof_idx)
{
 ...
 */
static nxpTfaProfileList_t *tfaContGet1stProfList(nxpTfaContainer_t *cont)
{
	nxpTfaProfileList_t *prof; 
 ...
 */
static nxpTfaLiveDataList_t *tfaContGet1stLiveDataList(nxpTfaContainer_t *cont)
{
 ...
 */
nxpTfaDeviceList_t *tfaContDevice_v1(nxpTfaContainer_t *cnt,  int dev_idx)
{
 ...
 */
static nxpTfaProfileList_t *tfaContNextProfile(nxpTfaProfileList_t *prof) 
{
 ...
 */
static nxpTfaLiveDataList_t *tfaContNextLiveData(nxpTfaLiveDataList_t *livedata) 
{
...
/* wrapper function */
void get_hw_features_from_cnt_v1(struct tfa_device *tfa,  int *hw_feature_register)
{
...
/* wrapper function */
void get_sw_features_from_cnt_v1(struct tfa_device *tfa,  int sw_feature_register[2])
{
...
enum Tfa98xx_Error tfa_set_filters_v1(struct tfa_device *tfa,  int prof_idx) 
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	nxpTfaProfileList_t *prof = tfaContGetDevProfList (tfa->cnt,  tfa->dev_idx,  prof_idx); 
	unsigned int i; 
