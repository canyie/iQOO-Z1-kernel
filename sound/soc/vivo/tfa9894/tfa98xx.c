...
static const char *default_fw_name = "tfa98xx.cnt";
...
void tfa98xx_read_status_registers(struct tfa98xx *tfa98xx, int enable);

static enum Tfa98xx_Error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int vstep);

#ifndef CONFIG_SND_SOC_TFA9874_OR_HAPTIC
...
static int tfa98xx_boot_init(int force);
static int tfa98xx_boot_cali(int force);
...
static struct tfa98xx_calib tfa98xx_cal = {
	.calibrate = tfa98xx_boot_cali,
	.mi2s_ready = 0,
};

struct tfa98xx_calib *get_tfa_cal(void)
{
	return &tfa98xx_cal;
};
			...
			int set_val, struct snd_kcontrol **tfa_control)
{
	...
	if (!tmp_control) {
		list_for_each_entry(tmp_control, &codec->component.card->snd_card->controls, list)
			if (strnstr (tmp_control->id.name, ctl_name, strlen(tmp_control->id.name))) {
				break;
			}
...
static int i2s_clk_enable_disable(int enable)
{
	int ret = 0;

	pr_info("[SmartPA-%d]mi2s_clk_enable:set Vivo MI2S Clock\n", __LINE__);

	ret = find_control_set_valu("Vivo MI2S Clock", !!enable, &mi2s_clk_control);
...
static int tfa98xx_check_Mtp(struct tfa98xx *tfa98xx)
{
	...
	status = TFA_GET_BF(tfa98xx->tfa, PWDN);
	if (status) {
		pr_info("[SmartPA-%d] tfa check_Mtp PLL in Powerdown, do coldStartup\n", __LINE__);
		tfaRunColdStartup_v1(tfa98xx->tfa, tfa98xx->profile);
		err = tfa98xx_get_mtp_v1(tfa98xx->tfa, &value);
		pr_info("[SmartPA-%d]tfa check_Mtp: now need to powerdown tfa\n", __LINE__);
	...
	if (err != Tfa98xx_Error_Ok) {
		pr_err("[SmartPA-%d]tfa check_Mtp: Unable to check DSP access: %d\n", __LINE__, err);
		return -EIO;
	}
	
	mtpex = (value & 2) >> 1;
	pr_info("[SmartPA-%d] tfa check_Mtp: value: 0x%x, MTPEX : %d\n", __LINE__, value, mtpex);
	return mtpex;
}
...
static int tfa98xx_reset_Mtp(struct tfa98xx *tfa98xx)
{
	...
	pr_info("tfa98xx_reset_Mtp enter\n");
	mutex_lock(&tfa98xx->dsp_lock);


	if (TFA_GET_BF(tfa98xx->tfa, PWDN)) {
		pr_info("[SmartPA-%d] tfa reset_Mtp() PLL in Powerdown, do coldStartup\n", __LINE__);
		tfaRunColdStartup_v1(tfa98xx->tfa, tfa98xx->profile);
	}

	tfa98xx_dsp_system_stable_v1(tfa98xx->tfa, &stable);
	if (stable == 0) {
	...
	tfa_calibrate_v1(tfa98xx->tfa);
	
	mutex_unlock(&tfa98xx->dsp_lock);

	pr_info("tfa98xx_reset_Mtp exit\n");
...
static int tfa98xx_calibration_done(int valu, struct tfa98xx *tfa98xx)
{
	...
	return (valu < tfa98xx->para.imped_min || valu > tfa98xx->para.imped_max) ? 0 : 1;

}

static int tfa98xx_boot_init_each(struct tfa98xx *tfa98xx, int force)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	int imped_valu = 0;
	int cal_profile = -1;
	int ret, scaling_factor;
	
	if (!tfa98xx) {
		pr_err("%s tfa98xx NULL.\n", __func__);
		return -EINVAL;
	}

	usleep_range (5000, 5000); /*I2C operation must be delayed 5ms after I2S*/
	pr_info("tfa98xx SmartPA boot_init.\n");

	cal_profile = tfaContGetCalProfile_v1(tfa98xx->tfa);
	if (cal_profile >= 0) {
		tfa98xx->profile = cal_profile;
	} else{
		ret = -1;
		pr_err ("%s cal_profile error,cal_profile:%d .\n", __func__, cal_profile);
		return ret;
	}
	...
	if (tfa_dev_mtp_get(tfa98xx->tfa, TFA_MTP_EX)) {
		pr_info ("%s mtpEx already success.\n", __func__);
		err = tfa_dsp_get_calibration_impedance_v1(tfa98xx->tfa);
		if (err) {
			pr_err ("tfa98xx tfa_dsp_get_calibration_impedance  0x%x \n", err);
			return err;
		}
	} else{
		tfa98xx->dsp_init = TFA98XX_DSP_INIT_FAIL;
		pr_err ("%s tfa98xx Calibrate failed.\n", __func__);
	}
	...
	if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_FAIL) {
		...
		if (!tfa98xx->calibration)
			pr_err("[SmartPA-%d] tfa boot_init:calibration impedance %02d.%02d is irrational\n", 
					__LINE__, (int)imped_valu/scaling_factor, (((int)imped_valu*1000)/scaling_factor)%1000);
	} else {
		tfa98xx->imped_val = 0;
	...
	if (tfa98xx->calibration) {
		pr_info("tfa98xx Calibration value is : %02d.%02d ohm\n", (int)imped_valu/scaling_factor, (((int)imped_valu*1000)/scaling_factor)%1000);
		tfa98xx->dsp_init = TFA98XX_DSP_INIT_STOPPED;
	} else {
		tfa98xx_reset_Mtp(tfa98xx);
		usleep_range(5000, 5000);
		pr_err("%s boot_init:calibrate failed.\n", __func__);
#if defined(VIVO_PROJECT_MODEL)
/*GDPR code*/
#else
/* not GDPR code start */
#ifdef CONFIG_FMEA
		writeDatas("1800", "1800_1", "tfa98xx calibrate error %02d.%02d",
			(int)imped_valu/scaling_factor, (((int)imped_valu*1000)/scaling_factor)%1000);
#endif
#ifndef CONFIG_RSC_VAUDIT
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SMART_PA, 0,
				"tfa98xx calibrate error %02d.%02d.",
			(int)imped_valu/scaling_factor,
			(((int)imped_valu*1000)/scaling_factor)%1000);
#endif
/* not GDPR code end */
#endif
		tfa98xx->dsp_init = TFA98XX_DSP_INIT_FAIL;
...
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	struct tfa98xx *temp, *tfa98xx = NULL;

	mutex_lock(&tfa98xx_mutex);
	list_for_each_entry_safe(tfa98xx, temp, &tfa98xx_device_list, list) {
		err |= tfa98xx_boot_init_each(tfa98xx, force);
	}
	mutex_unlock(&tfa98xx_mutex);
...
static inline char *tfa_cont_profile_name(struct tfa98xx *tfa98xx, int prof_idx)
{
	if (tfa98xx->tfa->cnt == NULL)
		return NULL;
	return tfaContProfileName_v1(tfa98xx->tfa->cnt, tfa98xx->tfa->dev_idx, prof_idx);
}
...
static void __tfa98xx_inputdev_check_register(struct tfa98xx *tfa98xx, bool unregister)
{
	...
	for (i = 0; i < tfa_cnt_get_dev_nprof(tfa98xx->tfa); i++) {
		if (strnstr(tfa_cont_profile_name(tfa98xx, i), ".tap", strlen(tfa_cont_profile_name(tfa98xx, i)))) {
			tap_profile = true;
...
int tfa9894_check_mtp_dbg(void);
int tfa9894_reset_mtp_dbg(void);
int tfa9894_init_dbg(char *buffer, int size);
struct tfa98xx_msg {
	char msgs[256];
	char reserved[252];
	int msg_result;
...
static int tfa98xx_dbgfs_temp_get(void *data, u64 *val)
{
	...
	mutex_lock(&tfa98xx->dsp_lock);
	*val = tfa98xx_get_exttemp_v1(tfa98xx->tfa);
	mutex_unlock(&tfa98xx->dsp_lock);
...
static int tfa98xx_dbgfs_temp_set(void *data, u64 val)
{
	...
	mutex_lock(&tfa98xx->dsp_lock);
	tfa98xx_set_exttemp_v1(tfa98xx->tfa, (short)val);
	mutex_unlock(&tfa98xx->dsp_lock);
									...
									loff_t *ppos)
{
	 ...
	 * purpose
	 */
	ret = tfa98xx_get_mtp_v1(tfa98xx->tfa, &status);
	if (ret) {
		ret = -EIO;
	...
	ret = tfaRunSpeakerCalibration_v1(tfa98xx->tfa);
	if (ret) {
		ret = -EIO;
									...
									size_t count, loff_t *ppos)
{
	...
	pr_info("tfa98xx_dbgfs_rpc_send count %d\n", (int)count);
	...
	if (tfa98xx->tfa->is_probus_device) {
		err = send_tfa_cal_apr(msg_file->data, msg_file->size, false);
		if (err) {
		...
		if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
			error = tfaContWriteFile_v1(tfa98xx->tfa, msg_file, 0, 0); /* int vstep_idx, int vstep_msg_idx both 0 */
			if (error != Tfa98xx_Error_Ok) {
										...
										loff_t *pos)
{
	...
	if (!tfa98xx || !tfa98xx_device_count) {
		return -ENOMEM;
	}

	devicenum |= devicenum << (tfa98xx_device_count - 1);
...
static ssize_t tfa98xx_dbgfs_fw_debug_read(struct file *file,
		char __user *buf, size_t count, loff_t *pos)
{
	const int size = 512;
	char buffer[size];
	int n = 0;
	const char *str = "Note: test for debug fw or reset for default fw\n"  \
		"step1) 'push tfa98xx.cnt to /vendor/firmware/tfa98xx_debug.cnt'\n" \
		"step2) 'echo test > fw-debug' or 'echo reset > fw-debug'\n\n";

	pr_info("fw version: %.2s%.2s\n", tfa98xx_container->version,
		tfa98xx_container->subversion);
	n += scnprintf(buffer+n, size-n, "fw version: %.2s%.2s\n\n",
		tfa98xx_container->version, tfa98xx_container->subversion);
	n += scnprintf(buffer+n, size-n, "%s", str);
	buffer[n] = 0;
	return simple_read_from_buffer(buf, count, pos, buffer, n);

}

static ssize_t tfa98xx_dbgfs_fw_debug_write(struct file *file,
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	struct i2c_client *i2c = file->private_data;
	struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
	nxpTfaContainer_t *old_tfa98xx_container = NULL;
	enum Tfa98xx_Error ret;
	char buf[32];
	...
	/* Compare strings, excluding the trailing \0 */
	if (!strncmp(buf, test_cmd, sizeof(test_cmd) - 1)) {
		fw_name = "tfa98xx_debug.cnt";
	} else if (!strncmp(buf, reset_cmd, sizeof(reset_cmd) - 1)) {
		fw_name = default_fw_name;
	} else {
		return -EINVAL;
	}

	pr_info("cmd %s fw_name: %s\n", buf, fw_name);

	mutex_lock (&tfa98xx_mutex);
	old_tfa98xx_container = tfa98xx_container;
	...
	mutex_lock (&tfa98xx_mutex);
	if (!ret) {
		if (old_tfa98xx_container)
			kfree(old_tfa98xx_container);
	} else {
		tfa98xx_container = old_tfa98xx_container;
	}
	mutex_unlock (&tfa98xx_mutex);
...
static const struct file_operations tfa98xx_dbgfs_fw_debug_fops = {
	.open = simple_open,
	.read = tfa98xx_dbgfs_fw_debug_read,
	.write = tfa98xx_dbgfs_fw_debug_write,
	.llseek = default_llseek,
};
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
	...
	debugfs_create_file("fw-debug", S_IRUGO|S_IWUGO, tfa98xx->dbg_dir,
						i2c, &tfa98xx_dbgfs_fw_debug_fops);
...
static int add_sr_to_profile(struct tfa98xx *tfa98xx, char *basename, int len, int profile)
{
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if ((len == bprof->len) && (0 == strncmp(bprof->basename, basename, len))) {
			...
			sr = tfa98xx_get_profile_sr_v1(tfa98xx->tfa, profile);
			if (!sr) {
...
static int tfa98xx_get_ctl_mute(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.integer.value[0] = tfa98xx->mute_state;

	return 0;
}
static int tfa98xx_set_ctl_mute(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct tfa98xx *tfa98xx = snd_soc_codec_get_drvdata(codec);
	...
	if (!tfa98xx->calibration) {
		pr_err("%s SmartPA Calibration failed. Not open.\n", __func__);
		return 0;
	}
	if (mute) {
		...
		mutex_lock(&tfa98xx->dsp_lock);
		tfa98xx_read_status_registers(tfa98xx, 0);
		tfa_dev_stop (tfa98xx->tfa);
		tfa98xx->dsp_init = TFA98XX_DSP_INIT_STOPPED;
		mutex_unlock(&tfa98xx->dsp_lock);
		usleep_range(2000, 2000); /* Waiting for i2s stability @zgbing */
	} else {
	#if 0
		if (stream == SNDRV_PCM_STREAM_PLAYBACK)
			tfa98xx->pstream = 1;
			...
			tfa98xx->cstream = 1;
	#endif
		pr_info ("tfa98xx open.\n");

		/* Start DSP */
		if (tfa98xx->tfa_number == 2) {
			if ((tfa98xx->flags & TFA9894_FLAG_CHIP_SELECTED) &&
				(tfa98xx->dsp_init != TFA98XX_DSP_INIT_PENDING)) {
	#if IS_ENABLED(CONFIG_SND_SOC_SAMSUNG_EXYNOS9630_TFA9894)
				usleep_range(5000, 5000);
	#else
				...
				tfa98xx_dsp_init(tfa98xx);
				mutex_unlock(&tfa98xx_mutex);
				tfa98xx_read_status_registers(tfa98xx, 1);
				//dev_info (&tfa98xx->i2c->dev, "%s: state1: %d\n", __func__, mute);
			}
		} else {
			if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_PENDING) {
						...
						tfa98xx_read_status_registers(tfa98xx, 1);
						//dev_info (&tfa98xx->i2c->dev, "%s: state2: %d\n", __func__, mute);
					}
		}
	}
	tfa98xx->mute_state = mute;
							...
							struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		ucontrol->value.integer.value[tfa98xx->tfa->dev_idx] =
				tfacont_get_max_vstep_v1(tfa98xx->tfa, profile)
				- vstep - 1;
	}
	mutex_unlock(&tfa98xx_mutex);
							...
							struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		vsteps = tfacont_get_max_vstep_v1(tfa98xx->tfa, profile);
		...
		if (profile == tfa98xx->profile) {
			...
			mutex_lock(&tfa98xx->dsp_lock);
			tfa98xx_dsp_system_stable_v1(tfa98xx->tfa, &ready);
							...
							struct snd_ctl_elem_info *uinfo)
{
	...
	mutex_unlock(&tfa98xx_mutex);
	uinfo->value.integer.min = 0;
	uinfo->value.integer.max = max(0, tfacont_get_max_vstep_v1(tfa98xx->tfa, profile) - 1);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	//tfa98xx_selector = ucontrol->value.integer.value[0];
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if (selector == CHIP_SELECTOR_LEFT) {
			if (tfa98xx->i2c->addr == CHIP_LEFT_ADDR)
				...
				tfa98xx->flags &= ~TFA9894_FLAG_CHIP_SELECTED;
			pr_err("tfa_set_stereo_ ctl left flags is %#x, i2c device address = %#x\n",
			...
			if (tfa98xx->i2c->addr == CHIP_RIGHT_ADDR)
				...
				tfa98xx->flags &= ~TFA9894_FLAG_CHIP_SELECTED;
			pr_err("tfa_set_stereo_ ctl right flags is %#x, i2c device address = %#x\n",
					tfa98xx->flags, tfa98xx->i2c->addr);
		} else {
			tfa98xx->flags |= TFA9894_FLAG_CHIP_SELECTED;
		}
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		if (tfacont_get_max_vstep_v1(tfa98xx->tfa, prof))
	...
	tfa98xx_controls[mix_index].get = tfa98xx_get_ctl_mute;
	tfa98xx_controls[mix_index].put = tfa98xx_set_ctl_mute;
	mix_index++;
				...
				int num_dai)
{
	...
	if (dai_drv && num_dai > 0) {
		for (i = 0; i < num_dai; i++) {
			snprintf(buf, 50, "%s-%d", dai_drv[i].name, id);
			dai_drv[i].name = tfa98xx_devm_kstrdup(dev, buf);
			pr_info("%s dai_drv.name:%s\n", __func__, dai_drv[i].name);

			snprintf(buf, 50, "%s-%d", dai_drv[i].playback.stream_name, id);
			dai_drv[i].playback.stream_name = tfa98xx_devm_kstrdup(dev, buf);
			pr_info("%s dai_drv.name:%s\n", __func__, dai_drv[i].playback.stream_name);

			snprintf(buf, 50, "%s-%d", dai_drv[i].capture.stream_name, id);
	...
	if (widgets && num_widgets > 0) {
		for (i = 0; i < num_widgets; i++) {
			...
			if ((widgets[i].id == snd_soc_dapm_aif_in)
				...
				snprintf (buf, 50, "%s-%d", widgets[i].sname, id);
				widgets[i].sname = tfa98xx_devm_kstrdup (dev, buf);
				pr_info("%s widgets.sname:%s\n", __func__, widgets[i].sname);
			}
		}
	}
...
static struct snd_soc_dapm_widget tfa98xx_dapm_widgets_common[] = {
	/* Stream widgets */
	SND_SOC_DAPM_AIF_IN ("AIF IN", "SmartPA Playback", 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_OUT ("AIF OUT", "SmartPA Capture", 0, SND_SOC_NOPM, 0, 0),

	SND_SOC_DAPM_OUTPUT ("OUTL"),
	SND_SOC_DAPM_INPUT ("SPKL AEC"),
...
static struct snd_soc_dapm_widget tfa9888_dapm_inputs[] = {
	SND_SOC_DAPM_INPUT ("TFA DMIC1"),
	SND_SOC_DAPM_INPUT ("TFA DMIC2"),
	SND_SOC_DAPM_INPUT ("TFA DMIC3"),
	SND_SOC_DAPM_INPUT ("TFA DMIC4"),
};

static const struct snd_soc_dapm_route tfa98xx_dapm_routes_common[] = {
	{ "SmartPA", "Switch", "AIF IN" },
	{ "OUTL", NULL, "SmartPA" },
	{ "AIF OUT", NULL, "AEC Loopback" },

	/* Tx path */
	{ "AIF OUT", NULL, "SPKL AEC" },
...
static const struct snd_soc_dapm_route tfa9888_input_dapm_routes[] = {
	{ "AIF OUT", NULL, "TFA DMIC1" },
	{ "AIF OUT", NULL, "TFA DMIC2" },
	{ "AIF OUT", NULL, "TFA DMIC3" },
	{ "AIF OUT", NULL, "TFA DMIC4" },
};
...
enum Tfa98xx_Error tfa98xx_write_register16_v1 (struct tfa_device *tfa, 
...
/* not GDPR code start */
			...
			writeDatas("1800", "1800_1",
				"audio driver i2c: tfa9894 reg 0x%x, val 0x%x, ret %d.\n",
				subaddress, value, ret);
#endif
#ifndef CONFIG_RSC_VAUDIT
			rsc_vaudit(VAUDIT_AUDIO, AUDIO_IIC_BUS, 0,
				"tfa9894 reg 0x%x, val 0x%x, ret %d.", subaddress, value, ret);
#endif
/* not GDPR code end */
#endif
			i2c_error = 0;
...
enum Tfa98xx_Error tfa98xx_read_register16_v1 (struct tfa_device *tfa, 
					unsigned char subaddress, 
					unsigned short *val)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
void tfa98xx_read_status_registers(struct tfa98xx *tfa98xx, int enable)
{
	...
	ret = regmap_read (tfa98xx->regmap, 0, &reg);
	if (ret < 0) {
		pr_err("tfa98xx Unable to read sys_control0 register.\n");
		return ;
	}
	pr_info("tfa98xx %s sys_control0 register:0x%x \n", enable?"open":"close", reg);

	ret = regmap_read (tfa98xx->regmap, 1, &reg);
	if (ret < 0) {
		pr_err("tfa98xx Unable to read sys_control1 register.\n");
		return ;
	}
	pr_info("tfa98xx %s sys_control1 register:0x%x \n", enable?"open":"close", reg);

	ret = regmap_read (tfa98xx->regmap, 2, &reg);
	if (ret < 0) {
		pr_err("tfa98xx Unable to read sys_control2 register.\n");
		return ;
	}
	pr_info("tfa98xx %s sys_control2 register:0x%x \n", enable?"open":"close", reg);

	ret = regmap_read (tfa98xx->regmap, 4, &reg);
	if (ret < 0) {
		pr_err("tfa98xx Unable to read clock_control register.\n");
		return ;
	}
	pr_info("tfa98xx %s clock_control register:0x%x \n", enable?"open":"close", reg);

	ret = regmap_read (tfa98xx->regmap, 0x10, &reg);
	if (ret < 0) {
		pr_err("tfa98xx Unable to read status_flags0 register.\n");
		return ;
	}
	pr_info("tfa98xx %s status_flags0 register:0x%x \n", enable?"open":"close", reg);

	ret = regmap_read (tfa98xx->regmap, 0x11, &reg);
	if (ret < 0) {
		pr_err("tfa98xx Unable to read status_flags1 register.\n");
		return ;
	}
	pr_info("tfa98xx %s status_flags1 register:0x%x \n", enable?"open":"close", reg);

	ret = regmap_read (tfa98xx->regmap, 0x12, &reg);
	if (ret < 0) {
		pr_err("tfa98xx Unable to read status_flags2 register.\n");
		return ;
	}
	pr_info("tfa98xx %s status_flags2 register:0x%x \n", enable?"open":"close", reg);

	ret = regmap_read (tfa98xx->regmap, 0x14, &reg);
	if (ret < 0) {
		pr_err("tfa98xx Unable to read status_flags3 register.\n");
		return ;
	}
	pr_info("tfa98xx %s status_flags3 register:0x%x \n", enable?"open":"close", reg);
 ...
 */
enum Tfa98xx_Error tfa98xx_write_dsp (struct tfa_device *tfa,  int num_bytes, const char *command_buffer)
{
	int err;
	nxpTfaFileDsc_t *msg_file;
	...
	/* msg_file.name is not used */
	msg_file = kmalloc(num_bytes + sizeof(nxpTfaFileDsc_t), GFP_KERNEL);
	if (msg_file == NULL) {
...
tfa98xx_read_dsp (struct tfa_device *tfa,  int num_bytes, unsigned char *result_buffer)
{
	...
	error = send_tfa_cal_apr(buffer, num_bytes, true);
...
enum Tfa98xx_Error tfa98xx_read_data_v1 (struct tfa_device *tfa, 
				unsigned char reg, 
...
enum Tfa98xx_Error tfa98xx_write_raw_v1 (struct tfa_device *tfa, 
				int len, 
...
static int tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (tfa98xx_container == NULL) {
		...
		tfa_err = tfa_load_cnt_v1 (container, container_size);
	...
	*/
	...
	tfa_cnt_get_app_name_v1 (tfa98xx->tfa, tfa98xx->fw.name);
	...
	if (tfa98xx->tfa->dev_idx == 0 && first == 0)
...
/* not GDPR code start */
#ifdef CONFIG_FMEA
		writeDatas("1800", "1800_1", "Smart pa firmware error %d", ret);
#endif
#ifndef CONFIG_RSC_VAUDIT
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SMART_PA, 0,
	...
	*/
	...
	for (prof = 0; prof < nprof; prof++) {
		if (0 == strncmp (basename, tfa_cont_profile_name (tfa98xx, prof), len)) {
			 ...
			 * and enforce this.
			 */
			sr = tfa98xx_get_profile_sr_v1 (tfa98xx->tfa, prof);
			if (!sr)
							...
							struct snd_soc_dai *dai)
{
	...
	if (!smart_pa_switch_enable) {
		dev_err (codec->dev, "tfa98xx_hw_params smart_pa_switch_enable not enable\n");
		return 0;
	}
...
static const struct snd_soc_dai_ops tfa98xx_dai_ops = {
	...
	//.mute_stream = tfa98xx_mute, /*move to kcontrol mono mute*/
};
...
static int tfa98xx_probe (struct snd_soc_codec *codec)
{
	...
	snd_soc_dapm_ignore_suspend (dapm, "SPKL AEC");
	snd_soc_dapm_ignore_suspend (dapm, "SPKL OUT");
	if (tfa98xx->tfa_number == 2) {
		snd_soc_dapm_ignore_suspend (dapm, "Left SPKL AEC");
		snd_soc_dapm_ignore_suspend (dapm, "Left SPKL OUT");
		snd_soc_dapm_ignore_suspend (dapm, "Right SPKL AEC");
		snd_soc_dapm_ignore_suspend (dapm, "Right SPKL OUT");
	} else if (tfa98xx->tfa_number == 1) {
		snd_soc_dapm_ignore_suspend (dapm, "Left SPKL AEC");
		snd_soc_dapm_ignore_suspend (dapm, "Left SPKL OUT");
	}
		...
		struct device_node *np) 
{
	...
	ret = of_property_read_u32 (np, "vivo,tfa98xx-number", &temp);
	tfa98xx->tfa_number = (ret == 0) ? (int)temp : 1;
	pr_err("The tfa-number = %d, realnum=%d.\n", temp, tfa98xx->tfa_number);
...
int tfa9894_reset_mtp_dbg (void)
{
	...
	pr_info ("[SmartPA-%d] tfa reset_mtp_dbg: reset mtp Enter.\n", __LINE__);
	tfa98xx_mi2s_clk_enable ();
	
	list_all_tfa98xx (tfa98xx) {
		tfa98xx_reset_Mtp (tfa98xx);
		tfa_dev_stop (tfa98xx->tfa);
		pr_info ("[SmartPA-%d] tfa reset_mtp_dbg:\n", __LINE__);
	}
	
	tfa98xx_mi2s_clk_disable ();
	pr_info ("[SmartPA-%d] tfa reset_mtp_dbg: reset mtp complete.\n", __LINE__);
	return 0;
...
int tfa9894_init_dbg (char *buffer, int size)
{
	...
	list_all_tfa98xx (tfa98xx) {
		done |= (tfa98xx_calibration_done (tfa98xx->imped_val, tfa98xx) << i);
		i++;
		...
		n += scnprintf (buffer + n, size - n, 
					...
					course, fine, tfa98xx->para.imped_min / 1000, 
...
void tfa9894_get_client (struct i2c_client **client, unsigned char addr)
{
...
int tfa9894_read_freq_dbg (char *buffer, int size)
{
...
int tfa9894_check_mtp_dbg (void)
{
	...
	tfa98xx_mi2s_clk_enable ();
	list_all_tfa98xx (tfa98xx) {
		ret |= tfa98xx_check_Mtp (tfa98xx) << i;
		i++;
	}
	tfa98xx_mi2s_clk_disable ();
...
void tfa9894_read_prars_dbg (int temp[5], unsigned char addr)
{
	struct tfa98xx *tfa98xx, *prev = NULL;
	list_all_tfa98xx (tfa98xx) {
		prev = tfa98xx;
								...
								const struct i2c_device_id *id)
{
	...
	pr_info ("tfa98xx probe:addr = 0x%x\n", i2c->addr);
	...
	ret = dev_set_name(&i2c->dev, "%s-%x", "tfa98xx", i2c->addr);
	if (ret || ERR_PTR(ret))
		return ret;
	...
	vivo_set_codec_name(dev_name(&i2c->dev)); //baoqiang add for codec register for mtk
	if (gpio_is_valid (tfa98xx->irq_gpio) &&
		!(tfa98xx->flags & TFA98XX_FLAG_SKIP_INTERRUPTS)) {
...
#endif
	/* Register the sysfs files for climax debug access */
