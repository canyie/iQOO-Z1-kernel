...
/* not GDPR code start */
#ifdef CONFIG_FMEA
extern int writeData (char *modelId, char *filename, char *fmt);
...
void tfanone_ops_v1(struct tfa_device_ops *ops);
void tfa9872_ops_v1(struct tfa_device_ops *ops);
void tfa9874_ops_v1(struct tfa_device_ops *ops);
void tfa9912_ops_v1(struct tfa_device_ops *ops);
void tfa9888_ops_v1(struct tfa_device_ops *ops);
void tfa9891_ops_v1(struct tfa_device_ops *ops);
void tfa9897_ops_v1(struct tfa_device_ops *ops);
void tfa9896_ops_v1(struct tfa_device_ops *ops);
void tfa9890_ops_v1(struct tfa_device_ops *ops);
void tfa9895_ops_v1(struct tfa_device_ops *ops);
void tfa9894_ops_v1(struct tfa_device_ops *ops);
...
int tfa_get_calibration_info_v1(struct tfa_device *tfa, int channel)
{
...
{
	int value = tfa_get_bf_v1(tfa, TFA9912_BF_CFTAPPAT);
	int bitshift;
...
void tfa_set_query_info (struct tfa_device *tfa)
{
	...
	case 0: /* tfanone : non-i2c external DSP device */
		...
		tfa->spkr_count = 0;
		tfa->daimap = 0;
		tfanone_ops_v1 (&tfa->dev_ops); /* register device operations via tfa hal*/
		tfa->bus = 1;
		break;
	case 0x72:
		/* tfa9872 */
		...
		tfa9872_ops_v1 (&tfa->dev_ops); /* register device operations */
		break;
	case 0x74:
		/* tfa9874 */
		tfa->has_msg = 1;
		...
		tfa->is_probus_device = 1;
		tfa->daimap = Tfa98xx_DAI_TDM;
		tfa9874_ops_v1 (&tfa->dev_ops); /* register device operations */     
		break;
	case 0x88:
		/* tfa9888 */
		tfa->tfa_family = 2;
		tfa->spkr_count = 2;
		tfa->daimap = Tfa98xx_DAI_TDM;
		tfa9888_ops_v1 (&tfa->dev_ops); /* register device operations */
		break;
	case 0x97:
		/* tfa9897 */
		...
		tfa->spkr_count = 1;
		tfa->daimap = Tfa98xx_DAI_TDM;
		tfa9897_ops_v1 (&tfa->dev_ops); /* register device operations */
		break;
	case 0x96:
		/* tfa9896 */
		tfa->supportDrc = supportNo;
		tfa->spkr_count = 1;
		tfa->daimap = Tfa98xx_DAI_TDM;
		tfa9896_ops_v1 (&tfa->dev_ops); /* register device operations */
		break;
	case 0x92:
		/* tfa9891 */
		tfa->spkr_count = 1;
		tfa->daimap = (Tfa98xx_DAI_PDM | Tfa98xx_DAI_I2S);
		tfa9891_ops_v1 (&tfa->dev_ops); /* register device operations */
		break;
	case 0x91:
		/* tfa9890B */
		tfa->spkr_count = 1;
	...
	case 0x81:
		...
		tfa9890_ops_v1 (&tfa->dev_ops); /* register device operations */
		break;
	case 0x12:
		/* tfa9895 */
		tfa->spkr_count = 1;
		tfa->daimap = Tfa98xx_DAI_I2S;
		tfa9895_ops_v1 (&tfa->dev_ops); /* register device operations */
		break;
	case 0x13:
		/* tfa9912 */
		tfa->tfa_family = 2;
		tfa->spkr_count = 1;
		tfa->daimap = Tfa98xx_DAI_TDM;
		tfa9912_ops_v1 (&tfa->dev_ops); /* register device operations */
		break;
	case 0x94:
		/* tfa9894 */
		tfa->tfa_family = 2;
		tfa->spkr_count = 1;
		tfa->daimap = Tfa98xx_DAI_TDM;
		tfa9894_ops_v1 (&tfa->dev_ops); /* register device operations */
		break;
 ...
 */
static int tfa98xx_dev2family (int dev_type)
{
 ...
 */
enum Tfa98xx_DMEM tfa98xx_filter_mem_v1 (struct tfa_device *tfa, int filter_index, unsigned short *address, int channel)
{
...
* Used by the LTT
*/
static void tfa98xx_rev (int *major, int *minor, int *revision)
{
 ...
 */
enum Tfa98xx_Error tfa98xx_supported_saam_v1 (struct tfa_device *tfa, enum Tfa98xx_saam *saam)
{
	int features;
	enum Tfa98xx_Error error;

	if (tfa->support_saam == supportNotSet) {
		error = tfa98xx_dsp_get_hw_feature_bits_v1 (tfa, &features);
 ...
 */
enum Tfa98xx_Error tfa98xx_compare_features_v1 (struct tfa_device *tfa, int features_from_MTP[3], int features_from_cnt[3])
{
	...
	tfa98xx_dsp_system_stable_v1 (tfa, &status);
	if (!status)
	...
	value = tfa_read_reg_v1 (tfa, mtpbf) & 0xffff;
	features_from_MTP[0] = tfa->hw_feature_bits = value;

	/* Read SW features: */
	error = tfa_dsp_cmd_id_write_read_v1 (tfa, MODULE_FRAMEWORK, FW_PAR_ID_GET_FEATURE_INFO, sizeof (bytes), bytes);
	if (error != Tfa98xx_Error_Ok)
		return error; /* old ROM code may respond with Tfa98xx_Error_RpcParamId */

	tfa98xx_convert_bytes2data_v1 (sizeof (bytes), bytes, &features_from_MTP[1]);

	/* check if feature bits from MTP match feature bits from cnt file: */
	get_hw_features_from_cnt_v1 (tfa, &features_from_cnt[0]);
	get_sw_features_from_cnt_v1 (tfa, &features_from_cnt[1]);
...
enum Tfa98xx_Error tfa98xx_dsp_reset_v1 (struct tfa_device *tfa, int state)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error tfa98xx_dsp_system_stable_v1 (struct tfa_device *tfa, int *ready)
{
 ...
 */
enum Tfa98xx_Error tfa98xx_init_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	uint16_t value = 0;
	...
	/* Put DSP in reset */
	tfa98xx_dsp_reset_v1 (tfa, 1); /* in pair of tfaRunStartDSP () */
...
enum Tfa98xx_Error tfa98xx_dsp_write_tables_v1 (struct tfa_device *tfa, int sample_rate)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error tfa98xx_dsp_get_memory_v1 (struct tfa_device *tfa, int memoryType, 
...
enum Tfa98xx_Error tfa98xx_dsp_set_memory_v1 (struct tfa_device *tfa, int memoryType, 
 ...
 */
enum Tfa98xx_Error tfa98xx_get_mtp_v1 (struct tfa_device *tfa, uint16_t *value)
{
	int status;
	int result;
 ...
 */
void tfa98xx_key2_v1 (struct tfa_device *tfa, int lock)
{
...
void tfa2_manual_mtp_cpy_v1(struct tfa_device *tfa,  uint16_t reg_row_to_keep,
						uint16_t reg_row_to_set, uint8_t row)
{
	uint16_t value;
	enum Tfa98xx_Error error;
...
enum Tfa98xx_Error tfa98xx_set_mtp_v1 (struct tfa_device *tfa, uint16_t value, uint16_t mask)
{
	unsigned short mtp_old, mtp_new;
	int loop, status;
	enum Tfa98xx_Error error;

	error = tfa98xx_get_mtp_v1 (tfa, &mtp_old);
	...
	error = tfa98xx_dsp_system_stable_v1 (tfa, &status);
	if (error) {
		return error;
	}
	if (status == 0) {
 ...
 * start tfa
 */
int tfa_calibrate_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error error;
	
	pr_info ("tfa_calibrate\n");

	/* clear mtpex */
	error = tfa98xx_set_mtp_v1 (tfa, 0, TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_MSK);
	if (error)
		return error ;
...
void tfa98xx_set_exttemp_v1 (struct tfa_device *tfa, short ext_temp)
{
...
short tfa98xx_get_exttemp_v1 (struct tfa_device *tfa)
{
...
enum Tfa98xx_Error tfa98xx_set_volume_level_v1 (struct tfa_device *tfa, unsigned short vol)
{
	if (tfa->in_use == 0)
		return Tfa98xx_Error_NotOpen; 
...
enum Tfa98xx_Error
tfa98xx_set_mute_v1 (struct tfa_device *tfa, enum Tfa98xx_Mute mute)
{
	if (tfa->in_use == 0) {
		 ...
		 const unsigned char *bytes)
{
	...
	while (index  <  length) {
		...
		error = tfa98xx_write_raw_v1 (tfa, size, &bytes[index]); 
		if (error != Tfa98xx_Error_Ok)
			break; 
		index += size; 
	}
...
enum Tfa98xx_Error
tfa_dsp_patch_v1 (struct tfa_device *tfa, int patchLength, 
...
TFA_INTERNAL enum Tfa98xx_Error tfa98xx_wait_result_v1(struct tfa_device *tfa, int wait_retry_count)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
...
*/
void tfa98xx_convert_bytes2data_v1 (int num_bytes, const unsigned char bytes[], 
								int data[])
{
...
*/
void tfa98xx_convert_data2bytes_v1 (int num_data, const int data[], 
								unsigned char bytes[])
{
	int i; 			/* index for data */
	int k; 			/* index for bytes */
...
enum Tfa98xx_Error tfa_dsp_msg_write_v1 (struct tfa_device *tfa, int length, const char *buffer)
{
	int offset = 0; 
	...
	while ((error == Tfa98xx_Error_Ok) && (remaining_bytes > 0)) {
		...
		error = tfa98xx_write_data_v1 (tfa, FAM_TFA98XX_CF_MEM, 
...
enum Tfa98xx_Error tfa_dsp_msg_write_id_v1 (struct tfa_device *tfa, int length, const char *buffer, uint8_t cmdid[3])
{
	int offset = 0; 
	int chunk_size = ROUND_DOWN (tfa->buffer_size, 3); /* XMEM word size */
	int remaining_bytes = length; 
	...
	/* write cmd-id */
	error = tfa98xx_write_data_v1 (tfa, FAM_TFA98XX_CF_MEM, 3, (const unsigned char *)cmdid); 
...
*/
enum Tfa98xx_Error tfa_dsp_msg_status_v1 (struct tfa_device *tfa, int *pRpcStatus)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 

	error = tfa98xx_wait_result_v1(tfa, 2); /* 2 is only one try */
	...
	error = tfa98xx_check_rpc_status_v1 (tfa, pRpcStatus); 

	return error; 
}

const char *tfa98xx_get_i2c_status_id_string_v1 (int status)
{
	const char *p_id_str; 
...
enum Tfa98xx_Error tfa_dsp_msg_read_v1 (struct tfa_device *tfa, int length, unsigned char *bytes)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
	...
	while (num_bytes > 0) {
		...
		error = tfa98xx_read_data_v1 (tfa, FAM_TFA98XX_CF_MEM, burst_size, bytes + offset); 
		if (error != Tfa98xx_Error_Ok)
			return error; 
...
enum Tfa98xx_Error reg_read (struct tfa_device *tfa, unsigned char subaddress, unsigned short *value) //legen-mark
{
	enum Tfa98xx_Error error; 
...
enum Tfa98xx_Error tfa_dsp_msg_v1 (struct tfa_device *tfa, int length, const char *buf)
{
	enum Tfa98xx_Error error; 
	int tries, rpc_status = Tfa98xx_I2C_Req_Done; 

	/* write the message and notify the DSP */
	error = tfa_dsp_msg_write_v1 (tfa, length, buf); 
	if (error != Tfa98xx_Error_Ok)
		return error; 

	/* get the result from the DSP (polling) */
	for (tries = TFA98XX_WAITRESULT_NTRIES; tries > 0; tries--) {
		error = tfa_dsp_msg_status_v1 (tfa, &rpc_status); 
	...
	if (rpc_status != Tfa98xx_I2C_Req_Done) {
		...
		pr_err("DSP msg status: %d (%s)\n", rpc_status,
			tfa98xx_get_i2c_status_id_string_v1 (rpc_status));
#if defined(VIVO_PROJECT_MODEL)
/*GDPR code*/
#else
/* not GDPR code start */
#ifdef CONFIG_FMEA
		writeDatas("1800", "1800_1", "DSP msg status: %d (%s).",
			rpc_status, tfa98xx_get_i2c_status_id_string_v1 (rpc_status));
#endif
#ifndef CONFIG_RSC_VAUDIT
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SMART_PA, 0,
			"DSP msg status: %d (%s).", rpc_status,
			tfa98xx_get_i2c_status_id_string_v1 (rpc_status));
#endif
/* not GDPR code end */
#endif
 ...
 */
enum Tfa98xx_Error tfa_dsp_msg_id_v1 (struct tfa_device *tfa, int length, const char *buf, uint8_t cmdid[3])
{
	enum Tfa98xx_Error error; 
	int tries, rpc_status = Tfa98xx_I2C_Req_Done; 

	/* write the message and notify the DSP */
	error = tfa_dsp_msg_write_id_v1 (tfa, length, buf, cmdid); 
	if (error != Tfa98xx_Error_Ok)
		return error; 
...
/* not GDPR code start */
#ifdef CONFIG_FMEA
		writeDatas("1800", "1800_1", "dsp_msg_id(): rpc_status: %d, "
			"cmdid[0]:0x%x, cmdid[1]:0x%x, cmdid[2]:0x%x.\n",
			rpc_status, cmdid[0], cmdid[1], cmdid[2]);
#endif
#ifndef CONFIG_RSC_VAUDIT
		rsc_vaudit(VAUDIT_AUDIO, AUDIO_SMART_PA, 0,
			"dsp_msg_id(): rpc_status: %d, cmdid[0]:0x%x, cmdid[1]:0x%x, cmdid[2]:0x%x.",
			rpc_status, cmdid[0], cmdid[1], cmdid[2]);
#endif
/* not GDPR code end */
#endif
...
TFA_INTERNAL enum Tfa98xx_Error tfa98xx_check_rpc_status_v1 (struct tfa_device *tfa, int *pRpcStatus)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
	...
	/* 1) write DMEM = XMEM to the DSP XMEM */
	{
		...
		error = tfa98xx_write_data_v1 (tfa, FAM_TFA98XX_CF_CONTROLS, sizeof (buffer), buffer); 
	}
	if (error == Tfa98xx_Error_Ok) {
		/* read 1 word (24 bit) from XMEM */
		error = tfa98xx_dsp_read_mem_v1 (tfa, 0, 1, pRpcStatus); 
	}
...
/***************************** xmem only **********************************/
enum Tfa98xx_Error
tfa98xx_dsp_read_mem_v1 (struct tfa_device *tfa, 
		unsigned int start_offset, int num_words, int *pValues)
{
	...
	for (; num_bytes > 0; ) {
		...
		error = tfa98xx_read_data_v1 (tfa, FAM_TFA98XX_CF_MEM, burst_size, bytes); 
		if (error != Tfa98xx_Error_Ok)
			goto tfa98xx_dsp_read_mem_exit; 

		tfa98xx_convert_bytes2data_v1 (burst_size, bytes, p); 
...
enum Tfa98xx_Error
tfa98xx_dsp_write_mem_word_v1 (struct tfa_device *tfa, unsigned short address, int value, int memtype)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
	...
	tfa98xx_convert_data2bytes_v1 (1, &value, bytes); 
	error = tfa98xx_write_data_v1 (tfa, FAM_TFA98XX_CF_MEM, 3, bytes); 

	return error; 
}

enum Tfa98xx_Error tfa_cont_write_filterbank_v1 (struct tfa_device *tfa, nxpTfaFilter_t *filter)
{
	...
	for (biquad_index = 0; biquad_index  <  10; biquad_index++) {
		if (filter[biquad_index].enabled) {
			error = tfa_dsp_cmd_id_write_v1 (tfa, MODULE_BIQUADFILTERBANK, 
					biquad_index+1, /*start @1 */
					sizeof (filter[biquad_index].biquad.bytes), 
						filter[biquad_index].biquad.bytes); 
		} else {
			 error = Tfa98xx_DspBiquad_Disable_v1 (tfa, biquad_index+1); 
		}
		if (error) 
			return error; 
...
enum Tfa98xx_Error Tfa98xx_DspBiquad_Disable_v1(struct tfa_device *tfa, int biquad_index)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
	...
	tfa98xx_convert_data2bytes_v1 (BIQUAD_COEFF_SIZE, coeff_buffer, &bytes[nr]); 
...
enum Tfa98xx_Error tfa_dsp_cmd_id_write_v1 (struct tfa_device *tfa, 
...
enum Tfa98xx_Error tfa_dsp_cmd_id_write_read_v1 (struct tfa_device *tfa, 
												unsigned char module_id, 
												unsigned char param_id, int num_bytes, 
												unsigned char data[])
{
...
enum Tfa98xx_Error tfa_dsp_cmd_id_coefs_v1 (struct tfa_device *tfa, 
			   unsigned char module_id, 
			   unsigned char param_id, int num_bytes, 
			   unsigned char data[])
{
...
enum Tfa98xx_Error tfa_dsp_cmd_id_MBDrc_dynamics_v1 (struct tfa_device *tfa, 
			   unsigned char module_id, 
...
enum Tfa98xx_Error
tfa98xx_dsp_write_preset_v1 (struct tfa_device *tfa, int length, 
						const unsigned char *p_preset_bytes)
{
	...
	if (p_preset_bytes != NULL) {
		...
		error = tfa_dsp_cmd_id_write_v1 (tfa, MODULE_SPEAKERBOOST, 
 ...
 */
enum Tfa98xx_Error
tfa98xx_dsp_get_hw_feature_bits_v1 (struct tfa_device *tfa, int *features)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
	uint32_t value; 
	uint16_t mtpbf; 
	...
	if (tfa->hw_feature_bits != -1) {
		*features = tfa->hw_feature_bits; 
		...
		if (tfa->tfa_family == 1) {
			...
			tfa98xx_dsp_system_stable_v1 (tfa, &status); 
			if (!status) {
				get_hw_features_from_cnt_v1 (tfa, features); 
				/* skip reading MTP: */
...
enum Tfa98xx_Error
tfa98xx_dsp_get_sw_feature_bits_v1 (struct tfa_device *tfa, int features[2])
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
	...
	if (tfa->sw_feature_bits[0] != -1) {
		...
		if (tfa->tfa_family == 1) {
			...
			tfa98xx_dsp_system_stable_v1 (tfa, &status); 
			if (!status) {
				get_sw_features_from_cnt_v1 (tfa, features); 
				/* skip reading MTP: */
				return (features[0] == -1) ? Tfa98xx_Error_Fail : Tfa98xx_Error_Ok; 
			}
		}
		error = tfa_dsp_cmd_id_write_read_v1 (tfa, MODULE_FRAMEWORK, 
		...
		tfa98xx_convert_bytes2data_v1 (byte_size, bytes, features); 
	}
	return error; 
}

enum Tfa98xx_Error tfa98xx_dsp_get_state_info_v1 (struct tfa_device *tfa, unsigned char bytes[], unsigned int *statesize)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	int bSupportFramework = 0; 
	unsigned int stateSize = 9; 

	err = tfa98xx_dsp_support_framework_v1 (tfa, &bSupportFramework); 
	if (err == Tfa98xx_Error_Ok) {
		if (bSupportFramework) {
			err = tfa_dsp_cmd_id_write_read_v1 (tfa, MODULE_FRAMEWORK, 
				FW_PARAM_GET_STATE, 3 * stateSize, bytes); 
		} else {
			/* old ROM code, ask SpeakerBoost and only do first portion */
			stateSize = 8; 
			err = tfa_dsp_cmd_id_write_read_v1 (tfa, MODULE_SPEAKERBOOST, 
...
enum Tfa98xx_Error tfa98xx_dsp_support_drc_v1 (struct tfa_device *tfa, int *pbSupportDrc)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
	...
	*pbSupportDrc = 0; 
	...
	if (tfa->supportDrc != supportNotSet) {
		*pbSupportDrc = (tfa->supportDrc == supportYes); 
		...
		error = tfa98xx_dsp_get_sw_feature_bits_v1 (tfa, featureBits); 
		if (error == Tfa98xx_Error_Ok) {
...
enum Tfa98xx_Error
tfa98xx_dsp_support_framework_v1 (struct tfa_device *tfa, int *pbSupportFramework)
{
...
enum Tfa98xx_Error
tfa98xx_dsp_write_speaker_parameters_v1 (struct tfa_device *tfa, 
		int length, const unsigned char *p_speaker_bytes)
{
	...
	error = tfa98xx_dsp_support_drc_v1 (tfa, &bSupportDrc); 
	if (error != Tfa98xx_Error_Ok)
		return error; 
	...
	if (bSupportDrc) {
		...
		error = tfa_dsp_cmd_id_write_v1 (tfa, 
			MODULE_SPEAKERBOOST, 
			SB_PARAM_SET_AGCINS, 
			3, 
			bytes); 
...
enum Tfa98xx_Error
tfa98xx_dsp_write_config_v1 (struct tfa_device *tfa, int length, 
...
enum Tfa98xx_Error tfa98xx_dsp_write_drc_v1 (struct tfa_device *tfa, 
...
enum Tfa98xx_Error tfa98xx_powerdown_v1 (struct tfa_device *tfa, int powerdown)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
...
enum Tfa98xx_Error
tfa98xx_select_mode_v1 (struct tfa_device *tfa, enum Tfa98xx_Mode mode)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
...
int tfa_set_bf_v1 (struct tfa_device *tfa, const uint16_t bf, const uint16_t value)
{
...
int tfa_set_bf_volatile_v1 (struct tfa_device *tfa, const uint16_t bf, const uint16_t value)
{
	enum Tfa98xx_Error err; 
...
int tfa_get_bf_v1 (struct tfa_device *tfa, const uint16_t bf)
{
	enum Tfa98xx_Error err; 
	uint16_t regvalue, msk; 
	uint16_t value; 
...
int tfa_set_bf_value_v1 (const uint16_t bf, const uint16_t bf_value, uint16_t *p_reg_value)
{
	uint16_t regvalue, msk; 
...
uint16_t tfa_get_bf_value_v1 (const uint16_t bf, const uint16_t reg_value)
{
	uint16_t msk, value; 
...
int tfa_write_reg_v1 (struct tfa_device *tfa, const uint16_t bf, const uint16_t reg_value)
{
	enum Tfa98xx_Error err; 
...
int tfa_read_reg_v1 (struct tfa_device *tfa, const uint16_t bf)
{
	enum Tfa98xx_Error err; 
	uint16_t regvalue; 
 ...
 */
enum Tfa98xx_Error tfa_cf_powerup_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	int tries, status; 
	...
	for (tries = CFSTABLE_TRIES; tries > 0; tries--) {
		err = tfa98xx_dsp_system_stable_v1 (tfa, &status); 
...
static enum Tfa98xx_Error tfa98xx_aec_output (struct tfa_device *tfa, int enable)
{
	...
	if (tfa->tfa_family == 1)
		err = -tfa_set_bf_v1 (tfa, TFA1_BF_I2SDOE, (enable != 0)); 
	else {
 ...
 */
enum Tfa98xx_Error show_current_state_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	int manstate = -1; 
 ...
 *
 */
enum Tfa98xx_Error tfaRunSpeakerBoost_v1 (struct tfa_device *tfa, int force, int profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	int value; 

	pr_info ("tfaRunSpeakerBoost enter \n"); 
	
	if (force) {
		err = tfaRunColdStartup_v1 (tfa, profile); 
		if (err) 
			return err; 
	}
	...
	pr_info ("tfa Startup of device [%s] is a %sstart\n", tfaContDeviceName_v1 (tfa->cnt, tfa->dev_idx), value ? "cold" : "warm"); 

	/* cold start and not tap profile */
	if (value) {
		/* Run startup and write all files */
		err = tfaRunSpeakerStartup_v1 (tfa, force, profile); 
		if (err) 
			return err; 
...
enum Tfa98xx_Error tfaRunSpeakerStartup_v1 (struct tfa_device *tfa, int force, int profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	
	pr_info ("tfaRunSpeakerStartup enter \n"); 

	if (!force) { /* in case of force CF already runnning */
		err = tfaRunStartup_v1 (tfa, profile); 
		PRINT_ASSERT(err); 
		if (err)
			return err; 
	...
	err = tfaContWriteFilesProf_v1 (tfa, profile, 0); 
	if (err) {
 ...
 * Run calibration
 */
enum Tfa98xx_Error tfaRunSpeakerCalibration_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	int calibrateDone; 
	...
	if (TFA_GET_BF (tfa, MTPOTC) == 1) {
		tfa98xx_key2_v1 (tfa, 0); 
	}

	/* await calibration, this should return ok */
	err = tfaRunWaitCalibration_v1 (tfa, &calibrateDone); 
	if (err == Tfa98xx_Error_Ok) {
		err = tfa_dsp_get_calibration_impedance_v1 (tfa); 
		PRINT_ASSERT(err); 
	}

	/* When MTPOTC is set (cal = once) re-lock key2 */
	if (TFA_GET_BF (tfa, MTPOTC) == 1) {
		tfa98xx_key2_v1 (tfa, 1); 
	}

	return err; 
}

enum Tfa98xx_Error tfaRunColdboot_v1 (struct tfa_device *tfa, int state)
{
...
{
	return tfaContWritePatch_v1 (tfa); 
}
 ...
 */
enum Tfa98xx_Error tfaRunStartDSP_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	...
	if (err == Tfa98xx_Error_Ok) {
		err = tfa98xx_dsp_reset_v1 (tfa, 0); 
		PRINT_ASSERT(err); 
	}

	/* Sample rate is needed to set the correct tables */
	err = tfa98xx_dsp_write_tables_v1 (tfa, TFA_GET_BF (tfa, AUDFS)); 
	PRINT_ASSERT(err); 
 ...
 */
enum Tfa98xx_Error tfaRunStartup_v1 (struct tfa_device *tfa, int profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	nxpTfaDeviceList_t *dev = tfaContDevice_v1 (tfa->cnt, tfa->dev_idx); 
	int i, noinit = 0; 
	
	pr_info ("tfaRunStartup enter \n"); 
 ...
 */
enum Tfa98xx_Error tfaRunColdStartup_v1 (struct tfa_device *tfa, int profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	
	pr_info ("tfaRunColdStartup enter \n"); 
	...
	if (!tfa->is_probus_device) {
		/* force cold boot */
		err = tfaRunColdboot_v1 (tfa, 1); /* set ACS */
		PRINT_ASSERT(err); 
		if (err)
			return err; 
	}
 ...
 *
 */
enum Tfa98xx_Error tfaRunMute_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	int status; 
	int tries = 0; 
	...
	if (tfa->tfa_family == 1) {
		err = tfa98xx_set_mute_v1 (tfa, Tfa98xx_Mute_Amplifier); 
 ...
 *
 */
enum Tfa98xx_Error tfaRunUnmute_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 

	/* signal the TFA98XX to mute  */
	err = tfa98xx_set_mute_v1 (tfa, Tfa98xx_Mute_Off); 
 ...
 */
enum Tfa98xx_Error tfaRunWaitCalibration_v1 (struct tfa_device *tfa, int *calibrateDone)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
...
enum Tfa98xx_Error tfa_dev_start (struct tfa_device *tfa, int next_profile, int vstep)
{
	...
	pr_info ("tfa Active_profile:%s, next_profile:%s\n", 
			tfaContProfileName_v1 (tfa->cnt, tfa->dev_idx, active_profile), 
			tfaContProfileName_v1 (tfa->cnt, tfa->dev_idx, next_profile)); 
	...
	if (tfa->bus != 0)  { /* non i2c  */
		...
		err = tfaRunSpeakerBoost_v1 (tfa, 0, next_profile); 
		if (err != Tfa98xx_Error_Ok)
			goto error_exit; 
		...
		if ((next_profile != active_profile && active_profile >= 0)) {
			err = tfaContWriteProfile_v1 (tfa, next_profile, vstep); 
			if (err != Tfa98xx_Error_Ok)
				goto error_exit; 
		}
		 ...
		 */
		if (strnstr (tfaContProfileName_v1 (tfa->cnt, tfa->dev_idx, next_profile), ".standby", strlen(".standby")) != NULL) {
		...
		if ((TFA_GET_BF (tfa, CFE) != 0) && (vstep != tfa->vstep) && (vstep != -1)) {
			err = tfaContWriteFilesVstep_v1 (tfa, next_profile, vstep); 
			if (err != Tfa98xx_Error_Ok)
				goto error_exit; 
		}

		/* Always search and apply filters after a startup */
		err = tfa_set_filters_v1 (tfa, next_profile); 
		if (err != Tfa98xx_Error_Ok)
			goto error_exit; 
...
enum Tfa98xx_Error tfa_dev_stop (struct tfa_device *tfa)
{
	...
	/* powerdown CF */
	err = tfa98xx_powerdown_v1 (tfa, 1); 
	if (err != Tfa98xx_Error_Ok)
		return err; 
	...
	/*Make sure the MANSTATE is in PowerDown State */
	while ((TFA_GET_BF(tfa, MANSTATE) != 0) && (times++ < 20)) {
		pr_debug("%s: state machine goto powerdown mode.\n", __func__);
		err = tfa98xx_dsp_system_stable_v1(tfa, &ready);
	...
	if (times < 20) {
		pr_info("%s tfa98xx already PowerDown.\n", __func__);
	} else {
		pr_info("%s tfa98xx Not PowerDown.\n", __func__);
	}
 ...
 */
int tfa_reset_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok; 
	...
	if (!tfa->is_probus_device) {
		...
		tfa98xx_powerdown_v1 (tfa, 0); 
		/* for clock */
		err = tfa_cf_powerup_v1 (tfa); 
		PRINT_ASSERT(err); 
 ...
 */
enum Tfa98xx_Error
tfa98xx_write_data_v1 (struct tfa_device *tfa, 
		  ...
		  const unsigned char data[])
{
	...
	error = tfa98xx_write_raw_v1 (tfa, bytes2write, write_data); 
 ...
 */
enum Tfa98xx_Error tfa_dsp_get_calibration_impedance_v1 (struct tfa_device *tfa)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok; 
	...
	if (tfa_dev_mtp_get (tfa, TFA_MTP_OTC)) {
		pr_info ("tfa98xx Getting calibration values from MTP\n"); 
		...
		pr_info("%s tfa->mohm[0]: %d.\n", __func__, tfa->mohm[0]);
	} else {
		pr_info ("tfa98xx Getting calibration values from Speakerboost\n"); 
		...
		if (!tfa->is_probus_device) {
			...
			if (calibrateDone != 1) {
				...
				pr_debug ("tfa98xx Calibration has timedout!\n"); 
				error = Tfa98xx_Error_StateTimedOut; 
			}
		}
		...
		error = tfa_dsp_cmd_id_write_read_v1 (tfa, MODULE_SPEAKERBOOST, SB_PARAM_GET_RE25C, nr_bytes, bytes); 
		if (error == Tfa98xx_Error_Ok) {
			tfa98xx_convert_bytes2data_v1 (nr_bytes, bytes, data); 
...
int tfa_dev_probe (int slave, struct tfa_device *tfa)
{
	...
	if (tfa98xx_read_register16_v1 (tfa, 3, &rev) != Tfa98xx_Error_Ok) {
...
enum Tfa98xx_Error tfa_dev_set_state (struct tfa_device *tfa, enum tfa_state state)
{
	...
	case TFA_STATE_OPERATING:    /* Amp and Algo running */
		...
		if (tfa->sync_iv_delay) {
			...
			tfa98xx_dsp_reset_v1 (tfa, 1); 
			tfa98xx_dsp_reset_v1 (tfa, 0); 
			tfa->sync_iv_delay = 0; 
		}
		break; 
	...
	if (state & TFA_STATE_MUTE)
		tfa98xx_set_mute_v1 (tfa, Tfa98xx_Mute_Amplifier); 

	if (state & TFA_STATE_UNMUTE)
		tfa98xx_set_mute_v1 (tfa, Tfa98xx_Mute_Off); 
	...
	case TFA_MTP_RE25_PRIM:
		if (tfa->tfa_family == 2) {
			if ((tfa->rev & 0xFF) == 0x88)
				...
				value = tfa_get_bf_v1 (tfa, TFA9912_BF_R25C); 
			else
...
enum Tfa98xx_Error tfa_dev_mtp_set (struct tfa_device *tfa, enum tfa_mtp item, int value)
{
	...
	switch (item) {
	case TFA_MTP_OTC:
		err = tfa98xx_set_mtp_v1 (tfa, (uint16_t)value, TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_MSK); 
		break; 
	case TFA_MTP_EX:
		err = tfa98xx_set_mtp_v1 (tfa, (uint16_t)value, TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_MSK); 
		break; 
	case TFA_MTP_RE25:
	case TFA_MTP_RE25_PRIM:
		if (tfa->tfa_family == 2) {
...
int tfa_plop_noise_interrupt (struct tfa_device *tfa, int profile, int vstep)
{
	...
	if (tfa_irq_get (tfa, tfa9912_irq_stnoclk)) {
		...
		if (no_clk == 1) {
			...
			*/
			if (strnstr (tfaContProfileName_v1 (tfa->cnt, tfa->dev_idx, profile), ".saam", strlen(".saam"))) {
