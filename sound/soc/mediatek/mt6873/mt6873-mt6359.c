...
#if 1 //for typec voice mos test
static const char * const typec_usb_str[] = {"normal", "fix_usb"};
static const struct soc_enum typec_hph_usb_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(typec_usb_str),
			    typec_usb_str),
};
extern int max20328_notify_mos(int state);
static int typec_state;
static int typec_to_usb_for_mos_get(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{

	pr_debug("%s(), status %d\n", __func__, typec_state);
	ucontrol->value.integer.value[0] = typec_state;

	return 0;
}

static int typec_to_usb_for_mos_set(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	typec_state = ucontrol->value.integer.value[0];

	pr_info("%s() %d\n", __func__, typec_state);

	max20328_notify_mos(typec_state);
...
static const struct snd_kcontrol_new mt6873_mt6359_controls[] = {
	...
	SOC_ENUM_EXT("DL_MOS_TEST", typec_hph_usb_enum[0],
			typec_to_usb_for_mos_get, typec_to_usb_for_mos_set),
};
