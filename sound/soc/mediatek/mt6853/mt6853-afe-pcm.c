				      ...
				      snd_pcm_uframes_t frames)
{
	...
	if (rtd->cpu_dai->id == MT6853_MEMIF_DL1 || rtd->cpu_dai->id == MT6853_MEMIF_VUL12) {
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (rtd->cpu_dai->id == MT6853_MEMIF_DL1) {
		...
		if (rtd->cpu_dai->id == MT6853_MEMIF_VUL12) {
			vivo_audio_ktv_set_channels(substream->runtime->channels);
