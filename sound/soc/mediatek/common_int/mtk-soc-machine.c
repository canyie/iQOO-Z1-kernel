...
#endif
//yuchun.yang add for vivo codec begin
	{
...
static int mt_soc_snd_probe(struct platform_device *pdev)
{
	...
	if (vivo_codec_name) {
		for (i = 0; i < card->num_links; i++) {
			if (!strcmp(mt_soc_dai_component[i].name, "VIVOCODECPLAYBACK")) {
				dev_info(&pdev->dev, "%s:vivo_codec_name %s\n", __func__, vivo_codec_name);
				mt_soc_dai_component[i].codec_name = vivo_codec_name;
				mt_soc_dai_component[i].codec_dai_name = "VIVO-Codec-dai";
			}
		}
	}
	//yuchun.yang add end
